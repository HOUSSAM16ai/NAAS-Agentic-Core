...
> Last updated: **2026-07-16** | Branch: `claude/project-refactor-microservices-y9w2mq` (D-168: قتل الملفين الضخمين الأخيرين — `routes.py` **3,312→1,561 سطراً (−53%)** عبر مانيفست `api_sources.py` + 6 وحدات مستخرَجة حرفياً، والعقل `probability_tutor_brain.py` **2,441→جذر تركيب ~55 سطراً + 5 mixins** (`probability_brain/` + مانيفست `brain_sources.py`) · M10-S2.3: «كيف حسبنا 4» تُعلَّم على الـ port + بوّابة التكافؤ 14→**17 عقداً** + فحص BRAIN⊆TUTOR · قاعدة monkeypatch late-binding · deselect في ci.yml −3 (إصلاح جذري لاختبارات stategraph الثلاثة) — §6.141 · D-169: اكتشافات E2E الحي — سلك نظيف (`_sanitize_wire_context`) + فرض D-102 على كل الكواشف + هوية الإدمن (ISS-131؛ ISS-132 مفتوح بـ ADR) — §6.142. سبقه D-165/D-166/D-167 §6.140).
