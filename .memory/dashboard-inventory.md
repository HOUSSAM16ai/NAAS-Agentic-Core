...
| `110-microservices-step8-reasoning-agent.json` | `cogniforge-ms-step8-reasoning-agent` | Step 8: reasoning-agent live — startup_info, LLM backend, HTTP traffic, invocations (success/error/fallback), MCTS expansions by depth, LLM calls/errors, fallback responses, microservices health matrix (steps 4-8), Prometheus scrape health. 20+ panels. | 10s | Migration engineers · SRE |
| `120-microservices-step11-full-skills.json` | `cogniforge-ms-step11-full-skills` | **Step 11 (current):** Full Skills Pipeline live — pipeline mode distribution (full/partial/fallback), skill calls success rate, P95 duration, content-retrieval-skill intent distribution (retrieval/explanation/unknown), hit/miss rate, KB size, checkpointer writes/reads, active threads, microservices health matrix (steps 4-11), ISS-042 Service Token auth, DSPy planning duration. 15 panels. | 10s | Migration engineers · SRE |
