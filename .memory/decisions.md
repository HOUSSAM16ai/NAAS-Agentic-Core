...
**ودرسان من تشغيل البوّابة نفسها:** (1) إعلانُ حزمةٍ ناقصة قد يكشف **تعارضاً كامناً**:
`langchain-community` يتطلّب `pydantic-settings>=2.4.0` بينما الملفّ يُثبِّت `==2.2.1`،
فصار غير قابل للحلّ أصلاً — فحُلَّت متطلّبات **الثلاث عشرة خدمة** محلّياً بـ`pip
--dry-run` (كلّها تُحَلّ) بدل دورةِ CI لكلّ حزمة. (2) **بوّابة تفشل لسببٍ ليس منها
تُدرِّب الناس على تجاهلها**: خمس عشرة عاملة تُقلع buildx معاً = خمس عشرة سحبة مجهولة
من Docker Hub، فخسرت `planning-agent` على `Client.Timeout` وملفّها سليم تماماً.
الحلّ تقييد التوازي (`max-parallel: 5`) لا إعادة المحاولة في نفس الجدار.
