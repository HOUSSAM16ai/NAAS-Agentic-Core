...
**العطب الثالث — بوّابةٌ لا تستطيع أن تفشل، ونصفُ نتيجتها ضجيج.** `run 31308376950`
طبع `✗ Found 4964 new problems according to the checks applied` **وانتهى `success`**؛
ومن الـ4,964 كان **2,584** «Invalid type hints» بدرجة High — لأنّ حاوية `qodana-python`
**لا تُثبّت تبعيات المشروع**، فكل استيراد غير محلول يصير خطأ أنواع. رقمٌ بهذا الحجم لا
يُقرأ ولا يُصلَح، وهو المقياس الزومبي بصيغة أخرى (D-016). العلاج: `bootstrap` في
`qodana.yaml` يُثبّت `requirements-ci.txt` نفسه الذي تُثبّته `lint`/`guardrails`، ثمّ
خطّ أساس (`--baseline`) + `fail-threshold` فتصمت المشاكل القائمة و**تُحمِّر الجديدة
وحدها**. ⛔ **لا Qodana ولا CodeScene في `required-ci`** (يمتدّ D-234) — يفرضه بند رابع
في البوّابة.
