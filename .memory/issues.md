...
## ISS-197 (2026-08-29) — المرآة متوقّفة منذ سبعة أيام، والفشل يُعاد بدل أن يُقرأ — 🟡 مفتوحة: التشخيص مُصلَح، والفتح فعلٌ في إعدادات المستودع النظير
...
**⛔ ما لم يُعالَج ولا يُعالَج من هنا:** فتح القفل فعلٌ في إعدادات `Houssam-lab/NAAS-Agentic-Core`
(إلغاء `Require linear history` + تجاوز شرط الـPR لمالك `UPSTREAM_PAT`). هذه الجلسة تقرأ المرآة
ولا تدفع إليها ولا تملك أداةً لتغيير حماية الفرع — والغياب يُصرَّح ولا يُقرأ نجاحاً (D-206 L11).
والبديل — إعادة كتابة التاريخ ليصير خطّياً — **مرفوض بلا ADR**: يعيد كتابة 60 دفعة بمُعرّفات
جديدة فيُفقد التطابق الذي وُجدت المزامنة لأجله، ويناقض D-287.

**شرط الإغلاق:** `git ls-remote --heads` على المستودعين يُرجع **نفس الـSHA** لـ`refs/heads/main`.
⛔ وظيفةٌ خضراء ليست مرآةً متطابقة — الحكم على `ls-remote` لا على لون الوظيفة (§6.6).
