...
**الدليل بعد الدمج (`4c749d7b`):** `live-e2e` خضراء على `main` في **المرآة نفسها** — أوّل
مرّة منذ أن وصلتها `3176fe6` بـ`required-ci`. والمزامنة نقلت الدمج في ≈٢٥ ثانية، وطبعت
خطوةُ ISS-198 `parity status: out_of_contract` **ووظيفتها ناجحة** — تقيس ولا تحجب.

**⚠️ وثغرةٌ مقيسة في البرهان، تُسجَّل لأنّ الصمت عنها يُقرأ نجاحاً (D-206 L11):** المزامنة
**ثنائية الاتجاه**، وما أُثبت اليوم اتجاهٌ واحد. `bakabala27-svg → Houssam-lab` نقل حمولةً
حقيقية (`bdad701` → `4c749d7`). أمّا `Houssam-lab → bakabala27-svg` فوظيفته عملت وأنهت
خطواتها الأربع — لكنّ النظير كان **أصلاً** عند نفس الـSHA، فكان الدفع بلا حمولة. وقراءةُ
بيئة الوظيفتين تُثبت أنّ الإعداد سليم (`MIRROR_PAT: ***` في المرآة · `UPSTREAM_PAT: ***`
في الأصل — لكلٍّ سرُّ اتجاهه)، لكنّ **الإعداد ليس برهاناً تشغيلياً** (§6.6). فالسبب بنيوي:
كلّ العمل ينشأ في `bakabala27-svg`، والاتجاه العكسي طريقٌ مُعبَّدٌ لا تمرّ عليه سيّارات —
وإن انكسر لن يُكتشَف إلّا حين يُحتاج.

**وهذه الفقرة نفسها هي المسبار:** كُتِبت ودُفِعت من `Houssam-lab/main` مباشرةً، فوصولها إلى
`bakabala27-svg/main` هو الساق الثالثة للاتجاه المفقود — أو غيابُها هو العطب مكشوفاً.
