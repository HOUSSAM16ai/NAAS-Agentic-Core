...
3. **التشخيص قبل الشرح** — لا نشرح قبل أن نعرف أين الخلل. **مُنفَّذ بنيوياً (D-155/ISS-122)**: أول طلب مساعدة يتلقى probe تشخيصياً واحداً (`_build_diagnostic_probe` — صفر قيم محسوبة) قبل أي خطوة.
   **والحيرة لا تُهنَّأ أبداً (D-208 · ISS-149):** حيرةٌ مُعلَنة («لم أفهم؟») لا تُنتج `mastered` مهما قال التاريخ، وذِكرُ الطالب لمصطلحٍ **داخل سؤاله عنه** ليس برهانَ إتقانه. القانون يسري على **كلّ** عقلٍ يقرّر (`pedagogical_policy` · `socratic_evaluator` · `pedagogical_escalation` · `understanding_state`) — كان مفروضاً على اثنين فقط، فبقيت البوّابة خضراء بينما الطالب يُهنَّأ على حيرته ويُسجَّل في نموذجه المعرفي إتقانٌ لم يحدث. تحرسه `check_confusion_never_an_answer` (مرمىً موسَّع) + `check_understanding_evidence`.
...
6. **التقدّم قبل الإطناب** — الرد الجيد يحرّك الطالب خطوة واحدة صحيحة، لا الأطول. **مُنفَّذ بنيوياً (D-155)**: الإجابة الصحيحة تُعترف صراحةً وتتقدّم للسؤال الفرعي التالي؛ سُلّم البدائل واعٍ بالسؤال المعلّق والإنقاذ الكامل آخر الخيارات.
...
| من يعترف بإجابة الطالب الصحيحة ويتقدّم؟ | **D-155**: `_verify_numeric_answer` (أرقام من combo حصراً — «هل هي 14 من 165» بأي صياغة) + `_pending_focus_from_history` (السؤال المعلّق من قوالبنا) ⇒ اعتراف صريح «أحسنت! ✅» + التقدّم للسؤال الفرعي التالي — ممنوع إعادة سؤال ما أُجيب |
| من يفرض التشخيص قبل المحتوى؟ | **D-155**: `_build_diagnostic_probe` — أول طلب مساعدة ⇒ سؤال تشخيصي واحد بلا أي قيم محسوبة (الطالب يولّد بصيرة «البيضاء مستحيلة» بنفسه)، ثم انتظار المحاولة |
