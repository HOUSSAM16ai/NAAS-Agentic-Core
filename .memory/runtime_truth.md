...
| 32 | docker-compose.step3.yml | `docker-compose.step3.yml` | **ACTIVE (manual trigger)** | 3-service compose: postgres-orchestrator:5441 + redis-orchestrator:6380 + orchestrator-service:8006. Isolated volumes. OPENROUTER_API_KEY + TAVILY_API_KEY injected. Run: `docker compose -f docker-compose.step3.yml up -d`. |
| 33 | Ona automation — orchestrator-stack | `.ona/automations.yaml` | **ACTIVE** | service: `orchestrator-stack` (start/ready/stop). tasks: `health-probe`, `verify-stack`, `run-step3-tests`. Trigger: `gitpod automations service start orchestrator-stack`. |
| 34 | Grafana dashboard Step 3 | `observability/grafana/dashboards/60-microservices-step3-live.json` | **ACTIVE** | 20 panels. UID: cogniforge-ms-step3-live. Refresh: 10s. Covers: orchestrator health, routing distribution, LangGraph nodes, intent classification, fallback chain, memory/CPU, logs. |
...
| 36 | orchestrator-service (Step 3 activated) | `microservices/orchestrator_service/` | **DORMANT→ACTIVE (on demand)** | DORMANT by default in devcontainer. ACTIVE when `gitpod automations service start orchestrator-stack` or `docker compose -f docker-compose.step3.yml up -d`. /health returns `startup_state: ready\|degraded`. |
...
30. **`OUTBOX_RELAY_ENABLED=false` in Step 3.** The outbox relay loop is disabled in `docker-compose.step3.yml`. It will be enabled in Step 4 after verifying the full persistence path (D-006 compliance check).
