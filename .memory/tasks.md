...
> Last updated: 2026-07-28 | Branch: `claude/probability-exercise-2024-v7agr3`
...
### 🔴 التالي مباشرةً — M0 أمني (قبل أي توصيل لمُخطِّط بالأدوات)
- **`shell_tool.py` غير سليم أمنياً:** `ALLOWED_COMMANDS` مُعرَّفة **ولا تُستخدَم**؛ الحارس
  الفعلي قائمة منع + `shell=True` (تمرّ: `rm -fr /home` · `curl x|sh` · `$(...)` · سطر جديد).
  مُستدعاة حيّاً من `local_fallback.py:51`. اليوم الأمر داخلي فلا حقن — لكن أي مُخطِّط يختار
  الأمر يحوّلها إلى RCE. الإصلاح: argv + `shell=False` + إحياء قائمة السماح + سجن مسارات +
  حدود موارد. **إصلاح أمني لكود حيّ، لا ميزة.**
- ثم M1: عقد أدوات موحَّد (`shared/tool_contract/`) يلفّ السجلّات الثلاثة المنفصلة
  (`agent_tools` · MCP · `TOOL_REGISTRY` في الأوركستريتور) بتصنيف قدرة/أثر.
