...
| نيّة الطالب (Intent) | **D-186** (`shared/intent` مصدر قانوني واحد + بوّابة `check_intent_single_source` تجعل القائمة السادسة مستحيلة، والدَّين مُجمَّد يتقلّص فقط · «التعريف قبل المثال» · ما سُلِّم لا يُعاد · البؤرة اللاصقة تسبق probe الافتتاح · **الترانسكريبت عقدٌ تنفيذي** في `tests/transcripts/` · ISS-139) |
| تعدّد اللغات (Polyglot) | **D-185/ADR-006** (TypeScript — المرحلة الأولى: **العقود أنواعاً مُتحقَّقة في CI** (`frontend/types/api.d.ts` مُولَّد من العقود الـ13 + `tsc -p tsconfig.check.json`)؛ كود الواجهة **لم يُحوَّل بعد** لأن `next build` يشترط تبعيات تُعدِّل القفل. التسع الباقيات مقاعد موثّقة بشرط تبنٍّ في `EXTENSION_SEAMS.md §7` — **الحدّ هو العقد لا اللغة**) |
