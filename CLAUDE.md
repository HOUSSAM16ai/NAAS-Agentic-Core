...
- **Stateful Pedagogical Progression is Mandatory**: The AI tutor must never rely purely on stateless chat history or naive string inference for its decisions. TutorState is the single source of truth for the student's pedagogical journey. The Monolith local graph uses PedagogicalPolicyEngine linked to this state to strictly forbid loops and intent hijacking.
- **Stateful Pedagogical Progression is Mandatory**: The AI tutor must never rely purely on stateless chat history or naive string inference for its decisions.  is the single source of truth for the student's pedagogical journey (e.g., , , , ). The Monolith  uses  linked to this state to strictly forbid loops and intent hijacking.
