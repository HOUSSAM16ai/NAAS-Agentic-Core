...
## 6.125 المعلّم السقراطي: السلطة الرمزية + حارس التكرار + حارس الانحراف — المرحلة 1 (2026-06-24, ISS-117 / D-142)

> **الكارثة (transcript حيّ):** على تمرين الاحتمالات 2024، الطالب يقدّم استنتاجين **صحيحين**
> («الخضراء و الحمراء فقط» ثم «لونين 2 فقط» — أدرك أن البيضاء مستحيلة لأن عددها 2 < 3)، فيردّ
> النظام «أحسنت / محاولة جيدة» **ثم يُعيد طباعة نفس التلميح حرفياً ثلاث مرّات** بلا تقدّم. وقبلها
> ينجرف على أسئلة المفهوم («ماذا نقصد لم أفهم» ⇒ «لم أفهم أي مفهوم؟» + سؤال عام عن الحادثة A).
> نفس نمط الفشل الذي حاولت سلسلة D-113→D-141 ترقيعه transcript-بـ-transcript.

### الأسباب الجذرية الخمسة (وكيلا Explore + قراءة الكود)
1. **LLM فلاكي يحكم سؤالاً رمزياً مغلقاً.** `SocraticEvaluatorSkill` يفوّض «هل فهم الطالب؟»
   لنموذج مجاني ضعيف؛ لإجابة صحيحة مُتحقَّقة يُرجِع `understood=false` ⇒ يُطلَق فرع
   `understood=False` في `orchestrator_client._stream_socratic_evaluation` فيُعيد التلميح المُبرمَج
   «لاحظ نوع كل كرة: أيّ الألوان يكفي عددها لسحب 3 منها معاً؟ ابدأ بعدّ الألوان الممكنة فقط.».
2. **صفر dedup.** ذلك التلميح بلا أي حارس ضد إعادة بثّ نصّ موجود في التاريخ ⇒ تكرار حرفي ×3.
3. **الاعتراف ≠ التقدّم.** `PedagogicalPolicySkill.decide` يضبط `acknowledge=true` مع `action="socratic"`.
4. **`mastered` لا يُطلَق أبداً.** `PedagogicalEscalationSkill._has_understanding_evidence` يُرجِع False
   حين `evidence_markers` فارغة (المُنادي لا يملؤها).
5. **حالة مُعاد بناؤها من التاريخ (بلا حالة دائمة).** المفهوم النشط/الميزانية/الرُّتب المُسلَّمة
   كلها تُشتقّ كل دور بمسح نصّي فضفاض على نافذة 50 رسالة ⇒ انجراف (ذكرٌ سابق لـ«الحادثة a» يجمّد
   المفهوم)، ميزانية سقراطية عامة تتسرّب بين المفاهيم، والرُّتب المُسلَّمة تضيع خارج النافذة.

### المرحلة 1 — الإصلاحات الجراحية الجذرية (مُنفَّذة، fail-open، بلا علم، بلا تغيير مخطّط)
- **1A — السلطة الرمزية (يكسر تجاوز الـ LLM الفلاكي):** `socratic_evaluator_skill.deterministically_correct`
  (مطابقة إشارات متينة بتوحيد الهمزات/الألف عبر `_sig_norm`) صارت **حاكمة**: حين تكون الإجابة
  مُتحقَّقة صحيحة رمزياً، يُمنع على الـ LLM إخفاض `understood` (يرفع فقط). يُمرَّر
  `verified_correct` من المحرك الرمزي عبر `OrchestratorClient._verify_answer_against_combo(question, combo)`
  (يُكرّم الإجابة الصحيحة حتى لو أخطأ تشخيص المفهوم). ⇒ «الخضراء و الحمراء فقط»/«لونين 2 فقط»
  تُقيَّم `understood=true` ⇒ فرع الاعتراف + الخطوة الرمزية التالية (تقدّم، لا تكرار).
- **1B — حارس التكرار العام:** `OrchestratorClient._recently_emitted(text, history)` (احتواء/تداخل
  رموز ≥ 0.8 على آخر رسائل المساعد). قبل البثّ في `_stream_socratic_evaluation`: إن كان النصّ
  مُكرّراً ⇒ **يُصعّد إلى الحلّ الرمزي المتدرّج (`_build_symbolic_reveal`) بدل الإعادة الحرفية**.
- **1D — حارس الانحراف:** `semantic_property_skill._is_bare_confusion_definitional` يميّز «ماذا نقصد
  لم أفهم» (حيرة مجرّدة بلا تسمية مفهوم) عن «ما هو الاحتمال الشرطي» (تسمية مفهوم جديد). الحيرة
  المجرّدة **تُبقي المفهوم النشط** (لا تُعيد الضبط إلى «لم أفهم أي مفهوم؟»)؛ المفهوم المُسمّى
  الجديد يُعيد الضبط (لا انجراف للمفهوم القديم).

### القواعد الأربع الدائمة (D-142 — لا تُكسر بدون ADR)
1. **الأرقام/الصحّة من المحرك الرمزي** (combo) لا الـ LLM؛ الإجابة المُتحقَّقة صحيحة تتقدّم حتمياً —
   الـ LLM يرفع `understood` فقط، لا يخفضه.
2. **لا تكرار حرفي:** أي تلميح/سؤال مُرشَّح مُكرّر لرسالة مساعد سابقة ⇒ تصعيد للرُّتبة التالية.
...
- **Sandbox (الآن — pydantic/Postgres محجوبان §6.55):** stub-harness على المنطق الصرف +
  `tests/services/test_d142_semantic_tutor_phase1.py` (1A/1B/1D عبر الـ skills الحقيقية ومُساعِدات
  المُنسّق): «الخضراء و الحمراء فقط»/«لونين 2 فقط» ⇒ understood (يتجاوز خفض الـ LLM)، التلميح المُكرّر
  يُكشَف، «ماذا نقصد لم أفهم» تُبقي المفهوم. ruff check/format + runtime_truth --check خضراء.
- **Codespaces (E2E حيّ إلزامي):** `git pull` + restart؛ إعادة transcript الكارثة عبر WS بالدخولين
  الحقيقيين (`houssamannaba963@gmail.com` / `benmerahhoussam16@gmail.com`) مقابل OpenRouter+Supabase:
  الإجابات الصحيحة تُكافأ وتتقدّم، صفر تكرار حرفي، صفر انجراف، صفر كشف (14/165)، عربية فقط.
...
## 6.126 الإجابة الحسابية الحتمية للاحتمالات — قتل انحراف الحادثة A + التكرار (2026-06-24, ISS-117 / D-143 المرحلة 1)

> **transcript حيّ (المالك):** على تمرين BAC-2024، أسئلة الحادثة B (56) ومبدأ الضرب (11×10×9)
> وجداء الأرقام كلها تُجاب بمحتوى **الحادثة A** («نفس اللون / 14 / 165»)، ثم يتكرّر الإغلاق
> **حرفياً**. تأكيد جنائي من الإنتاج الحيّ (عبر جسر Supabase): المحادثة 788 كرّرت إغلاق الاستنفاد
> **3 مرّات**، و15 رسالة تحمل توقيع «14/165».

### الجذور (مؤكَّدة من الكود الحيّ)
- **RC-1**: بوّابة المصفوفة التصعيدية (`orchestrator_client` ~3290) تسبق المسار الحسابي (~3847)، و
  `_esc_compute` لا تستبعد «نضرب/ضربنا/حصلنا على» ⇒ «لماذا نضرب 11×10×9» يُختطَف لسُلّم الحادثة A.
- **RC-2**: المحرّك الحتمي يعرف **الحادثة A فقط** (تأليفات اللون) — لا نموذج لـ B(56)/C/D/X/E(X)/الشرطي.
- **RC-3**: `_detect_subpart_question` يربط «165»→total و«14»→sum فقط؛ «56»/«11×10×9» ⇒ اشتقاق الحادثة A
  الذي **يُلوّث التاريخ** فيُرجِع `detect_active_concept` لاحقاً `same_color`.
- **RC-4**: فرع `exhausted` في `pedagogical_escalation_skill` يُعيد نصّاً ثابتاً كل دور بلا dedup.

### نقد المالك (CTO) ← الاستجابة المعمارية
1. **«لا باب LLM في مسار الرياضيات»**: المرحلة 1 لا توجّه أي سؤال حسابي لـ LLM. الحادثة B (56) تُحسَب
   **حتمياً** (C(8,3)/C(11,3) من أرقام الكرات الرسمية). الحوادث غير المنمذجة (C/D/X/الأمل/الشرطي) ⇒
   **تأجيل حتمي صادق** يسمّي الحدث ويُبعده عن الحادثة A — **بلا أيّ رقم من LLM**.
2. **«عالِج الفهم لا الاستجابة»**: التصعيد مشروط بإشارة فهم حقيقية (`_verify_answer_against_combo` +
   `UnderstandingStateSkill` + `DialogueManagerSkill` D-142)، لا dedup وحده.
3. **«لا تُفرط في بناء المرحلة 2»**: المرحلة 2 (C/D/X/E(X)/الشرطي حتمياً) تُشحَن **حدثاً حدثاً**، كلٌّ
   خلف بوّابة القياس (`cogniforge_tutor_routing_total{event,outcome}`: drift→0، repeated→0).

### المُنفَّذ (المرحلة 1)
- **`ProbabilityCalculatorSkill.number_parity_counts(content)`** — محلّل حتمي لأرقام الكرات المرقّمة
  (odd/even/zero/total) — يُمكّن الحادثة B دون أيّ LLM. مُتحقَّق: 2بيضاء+4حمراء+5خضراء ⇒ odd=8 ⇒ 56/165.
- **`OrchestratorClient._build_probability_computational_answer`** — preempt حتمي **قبل** بوّابة المصفوفة:
  مبدأ العدّ (لماذا نضرب 11×10×9 ÷ !3) | الحادثة B (56/165) | تأجيل صادق (C/D/X/الأمل/الشرطي). topic-safe،
  حارس لصق (>300 حرف ⇒ للاسترجاع)، صفر LLM.
- **defense-in-depth**: `_esc_compute` += نضرب/ضربنا/ضرب/«حصلنا على»؛ `detect_active_concept` لا تنجرف
  لـ same_color عند سؤال حسابي؛ حارس dedup عند emit المصفوفة (RC-4) يُصعّد للحلّ الرمزي بدل الإعادة.
- **القياس**: `record_probability_routing(event, outcome)` (بوّابة المالك #3).

### القواعد الدائمة (D-143 — لا تُكسر بدون ADR)
1. **صفر LLM يُنتِج رقماً احتمالياً** — كل الأرقام من المحرّك الرمزي (نقد #1).
2. سؤال حسابي/رقمي **لا يصل سُلّم الحادثة A أبداً** — يُعالَج بالـ preempt الحتمي.
3. لا حدث يُجاب بعدد حدثٍ آخر؛ الحوادث غير المنمذجة ⇒ تأجيل صادق صريح، لا الحادثة A، لا LLM.
   التأجيل الصادق **لا يطبع رقم الحادثة A (14/165) إطلاقاً** — حتى للمقارنة (نقد #1، D-143.1).
4. التصعيد/الإعادة ⇒ تقدّم، لا تكرار حرفي (`_recently_emitted`). **مسار الإجابة الحسابية نفسه**
   يحترم هذا: عند تكرار سؤال حسابي، نتقدّم لتمثيلٍ حتميّ مختلف
   (`_probability_computational_variant`) ثم لمُوجِّه تقدّم (`_probability_computational_advance_prompt`)
   — صفر تكرار حرفي، صفر LLM (D-143.1، نقد #2).
5. توسّع المرحلة 2 مشروط ببقاء مقاييس `cogniforge_tutor_routing_total` خضراء (drift→0، repeated→0).

### D-143.1 (2026-06-25) — متابعة Stage 1: إغلاق ثغرة التكرار على المسار الحسابي + إصلاح الاختبار
بعد التحقّق من `ac8008b` تبيّن أنّ `_build_probability_computational_answer` كان يُعيد **النصّ نفسه
حرفياً** عند تكرار السؤال الحسابي (الكارثة عينها في transcript المالك: «نضرب 11×10×9» ثمّ «نضرب ثلاثة
أرقام» مرّتين) لأنّه يُبثّ مباشرةً بلا حارس تكرار. الإصلاح: (1) حارس `_recently_emitted` على المسار
الحسابي ⇒ تمثيل مختلف ثمّ مُوجِّه تقدّم؛ (2) نصّ التأجيل صار يحذف «14/165» و«**» (نقد #1 — لا رقم
الحادثة A، ولا markdown يكسر العبارة)؛ (3) إصلاح `tests/services/test_d143_probability_computational.py`
الذي كان يفكّ الـ tuple بترتيب معكوس (`event, text` بدل `text, event`) فكان **أحمر في CI**. مقياس جديد
`outcome="advanced"` عند التمثيل البديل. كلّه حتمي صفر-LLM.

### التحقق
- **Sandbox**: محلّل التكافؤ ⇒ 56/165 ✅؛ **fuzz: 1809 صياغة** (MSA+دارجة+فرنسي) ⇒ **صفر انحراف للحادثة A،
  صفر اختطاف للمرور**؛ `ruff`/`runtime_truth`/`validate_structure`/`py_compile 3.12` ✅؛
  `tests/services/test_d143_probability_computational.py` (CI، يستدعي الكود الحقيقي).
- **حيّ (جسر Supabase HTTPS:443)**: PostgreSQL 17.6؛ تأكيد الكارثة في الإنتاج (محادثة 788 ×3، 15 رسالة «14/165»).
- **Codespaces (إلزامي)**: `scripts/verify_d143_live.py` يُعيد transcript عبر WS الحقيقي + fuzz آلاف
  الصياغات vs OpenRouter+Supabase (الدخولان الحقيقيان). المرحلة 2 (C/D/X/E(X)/الشرطي) بعد خُضرة المقاييس.
...
## 6.103 Socratic Filtering Generalization - No-Answer OS Patch (2026-06-15, ISS-118)
> **المشكلة:** النظام كان يسمح بتسريب نتائج نهائية بصيغ جديدة غير تقليدية (مثل `نجمع الحالات الممكنة فقط: ... = 15` أو `الاحتمال = 15/364`)، مما يسمح للـ LLM ببناء حسابات مهلوسة لـ "بطاقة رقم 0" كأنها لون بناءً على مجموع خاطئ للكرات (C(14,3)=364). الكارثة لم تكن في استخراج ProbabilitySkill للكيانات (لأنه لم يستخرج بطاقة رقم 0 من معطيات التمرين)، بل في وهم الطلاقة الذي سمح للـ LLM بالتأليف.
> **الإصلاح المعماري:** تعميم طبقة الحجب السقراطية (AnswerRedactionSkill و ResponseSanitizer) لتكون *Agnostic* تماماً عن أي تمرين. تم توسيع `_CONCLUSION_RE` و `_FINAL_RESULT_RE` لتصطاد أي صيغة رياضية تنتهي بنتيجة، أو أي استنتاج نصي يحتوي `الاحتمال =` أو `من كل`. هذا التوجه المعماري يضمن أن أي تمرين جديد مستقبلاً (من بين ملايين التمارين) سيخضع لفلترة سقراطية صارمة دون الحاجة لكتابة كود مخصص له.


## Phase 1 Execution Complete (Doctrine-Driven Implementation)
The following foundational architectural invariants have been established in the active codebase as of Phase 1:
...
## 6.129 نهاية «آلة تفريغ الحلول» — Progressive Disclosure + حارس تكرار لا يَعمى + رياضيات LaTeX + بذرة M10-S2.1 (2026-07-02, ISS-121 / D-154)

> **الكارثة (screenshot + transcript حيّان بعد D-153):** الأرقام صارت صحيحة (14/165) لكن
> «كيف» ⇒ **تفريغ الحل الكامل** حتى «فاحتمال الحادثة A هو 14 من كل 165»؛ والإجابة الصحيحة
> («الحمراء و الخضراء») ⇒ «إجابتك في الطريق الصحيح —» + **نفس التفريغ حرفياً مرة ثانية**؛
> والصيغ ASCII تتبعثر بالـ bidi داخل RTL («= :(5 العدد)» و«10» يتيمة)؛ ونسخ التمرين على
> Samsung يُضاعف الأرقام («1\n1»). حكم المالك: «مستحيل أن يحل الطالب يوم الامتحان خاصة إذا
> تغيرت طبيعة الأسئلة» + مطلب «الانتقال التدريجي للخدمات المصغرة» (roadmap M10-S2).

### الجذور الأربعة (مؤكَّدة بتتبّع الكود + stub harness)
1. **procedure ⇒ تفريغ**: فرع procedure في كتلة حالة الفهم كان يستدعي `_build_symbolic_reveal`
   مباشرة — يخالف القانون الرابع («التلميح قبل الحل») ومقياس roadmap §7 «صفر كشف».
2. **عمى تحويل الحجب**: حارس التكرار (D-142) يقارن المرشَّح (بأرقام كاملة) مع history/
   `last_step_emitted` المحفوظَين **بعد** `_apply_final_answer_redaction` («14»→«؟» — D-113)
   ⇒ لا تطابق أبداً ⇒ يبثّ المكرَّر؛ ومسار التصعيد الوحيد كان reveal (قد يكون هو المكرَّر نفسه).
3. **بعثرة bidi**: `_fmt_comb` ASCII (`C(5,3) = (5×4×3)/(3×2×1) = 10`) داخل فقرة RTL.
4. **النسخ المضاعف**: `.katex-mathml` كان `user-select:none` فقط — التحديد الأصلي على
   Samsung يتجاهله فيلتقط annotation + المرئي معاً.

### الإصلاح (D-154 — 4 مسارات)
- **Track 1 — الكشف التدريجي البنيوي (صفر كشف)**: نية procedure تدخل السُّلّم من خطوة البسط
  (`_build_symbolic_step(combo, None)` — تنتهي بسؤال «كم عدد كل الطرق؟»)؛ الـ reveal محجوز
  **حصراً** لإنقاذ استنفاد الميزانية (D-129). كل ذيول البُناة الحتمية (`_build_symbolic_reveal`
  + `_build_probability_direct_explanation` بفرعيه) صارت **أسئلة توليد**: «الآن أمامك كل
  المكوّنات — ركّب الاحتمال **بنفسك**: البسط على المقام. فما قيمة P(A) التي تحصل عليها؟» —
  النسبة النهائية لا تُطبع أبداً؛ الطالب يولّدها (generation effect).
- **Track 2 — حارس تكرار محايد للحجب + «ممنوع بثّ مكرَّر»**: `_norm_for_dedup`
  (orchestrator_client) و`_norm` (dialogue_manager_skill) يستبدلان مقاطع الأرقام و«؟» بـ`#`
  بعد `normalize_ar` ⇒ النسخة المحجوبة المحفوظة ≡ المبثوثة الكاملة في المقارنة. ومسارا البثّ
  (`_stream_socratic_evaluation` عبر `_is_dup` + حارس حالة الفهم عبر `_d153_dup`) يمشيان
  **سلسلة بدائل**: reveal → خطوة البسط → خطوة النسبة → مُوجّه توليد حتمي — أول **غير مكرَّر**
  يُبثّ، وإلا يسقط النص للمسار التالي. صفر تكرار حرفي بنيوياً.
- **Track 3 — رياضيات LaTeX-only**: `_fmt_comb` يُخرج
  `$C_{n}^{k} = \dfrac{…\times…}{…\times…} = fav$` — KaTeX يُصيّرها LTR-معزولة (يقتل بعثرة
  bidi في كل القوالب الحتمية دفعة واحدة)، وتَنجو بنيوياً من حجب D-113 (صيغة `C_{n}^{k}` بلا
  أقواس لا تطابق `C(...)=N`). + `.katex-mathml { display:none !important }` في `globals.css`
  (مقايضة a11y موثَّقة — الطبقة المرئية + aria باقيتان).
- **Track 4 — بذرة M10-S2.1 (مطلب المالك)**: port مستقل stdlib
  `microservices/orchestrator_service/src/services/overmind/probability_tutor.py`
  (`parse_composition` محصَّن ضد نثر الحل + بوّابة مقامات LaTeX-aware + `deterministic_turn`
  سُلّم صفر-كشف بحارس dedup) موصول في `SynthesizerNode` خلف علم
  `ORCHESTRATOR_PROB_TUTOR_ENABLED` (افتراضي off). الحالة الصادقة (§6.6): **FLAGGED** حتى
  التحقق الحيّ في Codespaces.

### القواعد الدائمة (D-154 — لا تُكسر بدون ADR)
1. **النسبة النهائية لا تُطبع خارج وضع التحقق (M8) أبداً**: كل ذيل بُنَاة حتمي = سؤال توليد.
   الـ reveal لإنقاذ استنفاد الميزانية حصراً — أي توجيه نية مباشر إليه يُعيد آلة التفريغ.
2. **تطبيع الـ dedup محايد للحجب إلزامياً**: أي مقارنة تكرار جديدة تمرّ عبر `_norm_for_dedup`/
   `near_duplicate` (مقاطع `[\d؟?]+`→`#`) — المقارنة على نسخ غير متطابقة الحجب = عمى.
3. **ممنوع بثّ مكرَّر بنيوياً**: كل مسار بثّ يمشي سلسلة بدائل وأول غير مكرَّر يفوز؛ استنفادها
   ⇒ إسقاط النص للمسار التالي، لا بثّ المكرَّر.
4. **الرياضيات الحتمية LaTeX-only**: ممنوع كسور ASCII خام (`(4×3×2)/(3×2×1)`) في أي نص
   مواجه للطالب — تتبعثر bidi في RTL. `_fmt_comb` هو المصدر الوحيد للصيغ التأليفية.
5. **`.katex-mathml` يبقى `display:none`** — إعادته مرئياً تُعيد النسخ المضاعف على الموبايل.
6. **الـ port لا يستورد من `app/` أبداً** (قاعدة M10: نسخ مستقل)؛ ولا يُعلَن ACTIVE قبل التحقق
   الحيّ بالعلم المقلوب في Codespaces.

### التحقق (2026-07-02)
- **Sandbox** (pydantic/Postgres محجوبان — §6.55): stub harness — `_fmt_comb` LaTeX + الذيول
  أسئلة توليد (صفر «من كل 165») + نجاة الحجب (`redact_final_answers` حقيقية) + التطبيع المحايد
  (كامل ≡ محجوب) ✅؛ الـ port standalone: الملف الرسمي الكامل ⇒ 11/3/165/14، 4 أدوار متمايزة
  صفر تسريب، الطلب الثري ⇒ None، صفر imports من app ✅. بوّابة `check_pedagogical_os` (12 فحصاً
  منها 4 جديدة D-154) ✅ | 17 اختبار node frontend ✅ | ruff check/format ✅ | py_compile 3.12 ✅
  | runtime_truth --check ✅ | check_test_hygiene ✅. اختبارات جديدة:
  `test_iss121_progressive_disclosure.py` + `test_probability_tutor_port.py` +
  `iss121_math_display.test.mjs`؛ ومواءمة D-124/ISS-120 لعقد LaTeX/السلسلة (D-105: مواءمة لا تعطيل).
- **Codespaces (E2E حيّ إلزامي)**: إعادة الترانسكريبت عبر WS + المتصفح بالدخولين الحقيقيين:
  «كيف» ⇒ خطوة بسؤال لا تفريغ؛ الإجابة الصحيحة ⇒ اعتراف + خطوة **جديدة**؛ الصيغ KaTeX بلا
  بعثرة؛ النسخ بلا مضاعفة؛ ثم قلب `ORCHESTRATOR_PROB_TUTOR_ENABLED=1` والتحقق من عقدة
  الـ orchestrator وترقية FLAGGED.

### السلسلة (D-153 → D-154)
| Decision | الموضوع |
|----------|---------|
| D-153 | ISS-120: أسئلة-فقط للمحرك الرمزي + الحيرة ليست إجابة + دستور Pedagogical OS |
| **D-154** | **ISS-121: نهاية آلة التفريغ — Progressive Disclosure (صفر كشف) + dedup محايد للحجب + سلاسل «ممنوع مكرَّر» + رياضيات LaTeX + `.katex-mathml` مخفي + بذرة M10-S2.1 (FLAGGED)** |

---

## 6.130 «الاعتراف والتقدّم» — الحلقة التربوية تقرأ الطالب فعلاً (2026-07-02, ISS-122 / D-155)

> **الكارثة (transcript حيّ بعد D-154):** D-154 صحيح ميكانيكياً (صفر تفريغ، صفر تكرار حرفي)
> لكن الحلقة **صمّاء عن الطالب**: «هل هي 14 من 165» (الإجابة الصحيحة!) ⇒ إعادة سؤال ما
> أُجيب للتو؛ «هي 14 على 165» (صحيحة ثانيةً) ⇒ الكاروسيل الكامل بلا «أحسنت»؛ «كيف احل
> السؤال الأول» ⇒ حساب البسط كاملاً بلا سؤال تشخيصي؛ «نفس الشئ مع 11» ⇒ reveal يعيد كتلة
> البسط (superset)؛ «لم افهم العلاقة بين 14 و 165»/«لماذا حصلنا على 14 و 165» ⇒ بدائل
> سُلّم صمّاء. حكم المالك: «النظام ما زال يفكر بمنطق كيف أُجيب؟ بدل كيف أُعلّم؟».

### الجذور الخمسة (مؤكَّدة بالاستكشاف file:line)
1. **الإجابة الرقمية غير مرئية لأي طبقة تحقّق**: `_verify_answer_against_combo` إشارات
   ألوان/استحالة فقط؛ `deterministically_correct` بلا أرقام؛ المُقيّم LLM أعمى عمداً عن
   14/165 (حقن الحقائق يستبعدها — D-128 no-reveal)؛ `evidence_markers` لفظية.
2. **الاعتراض السقراطي يبتلع الأسئلة المفاهيمية**: يسبق كتلة D-124/D-125 فتسقط
   «العلاقة/لماذا» في سُلّم `_is_dup` الأصمّ — `_format_conceptual_relationship` لا يُبلَغ.
3. **لا بوّابة تشخيص قبل المحتوى**: فرع procedure يبثّ الخطوة المحسوبة فوراً.
4. **الـ reveal أول بدائل السُّلّم** ⇒ عند dup يُبثّ superset يعيد المعروض.
5. **لا ledger «سؤال معلّق/خطوات أُجيبت»** (`kc_progress` لا يُكتب أبداً).

### الإصلاح (D-155 — حتمي-أولاً، الحالة من الـ history، صفر تغيير مخطط DB)
- **Fix 1 — التحقّق الرقمي الرمزي**: `_verify_numeric_answer(answer, combo, pending_focus)`
  — الأرقام من combo حصراً (same/total/n/favs، ترجمة الأرقام العربية). الأحكام:
  `final_ratio` (بأي صياغة: 14/165، «14 من 165»، «14 على 165») | `step_correct` |
  `direction` («نفس الشئ مع 11») | None (تُترك للمُقيّم — لا اعتراف زائف).
  `_pending_focus_from_history` يشتق السؤال المعلّق من **قوالبنا بلا أرقام** في أحدث
  رسالة مساعد (تنجو من حجب D-113 بنيوياً). يُغذّي `verified_correct` (الرافعة الأحادية
  القائمة — الـ LLM لا يستطيع الخفض). **صفر تغيير في حقن حقائق الـ LLM** (D-128 يبقى).
- **Fix 2 — الاعتراف والتقدّم**: `final_ratio` ⇒ تأكيد حتمي بلا أرقام «أحسنت! ✅ إجابتك
  صحيحة تماماً…» + التقدّم للحادثة B بسؤال تشخيصي واحد («متى يكون جداء ثلاثة أعداد
  فردياً؟») — لا خطوة، لا reveal، لا كاروسيل؛ الجزئي ⇒ اعتراف + **الخطوة التالية غير
  المُجابة**. الحكم الرقمي يتجاوز إعادة تشكيل الـ DialogueManager.
- **Fix 3 — الهروب المفاهيمي**: بوّابة الاعتراض السقراطي تستثني
  `_detect_conceptual_question` + `_QUESTION_OPENERS_NOT_ANSWERS` (لماذا/ليش/علاش/كيف/
  ما الفرق — **ليست «هل»**: «هل هي 14 من 165» تبقى إجابة) ⇒ D-125 يُبلَغ أخيراً وسط
  الحوار؛ وداخل كتلة D-124 المفاهيمي يقصر مباشرةً على `_build_probability_direct_explanation`.
- **Fix 4 — سُلّم واعٍ بالسؤال المعلّق**: سلسلتا البدائل (`_is_dup` + `_d153_dup`)
  بالترتيب [الخطوة التالية غير المُجابة → الخطوة الأخرى → مُوجّه التوليد → **reveal
  أخيراً**] — يقتل تكرار الـ superset المُقنَّع.
- **Fix 5 — التشخيص قبل المحتوى**: `_build_diagnostic_probe(combo)` — سؤال تشخيصي واحد
  **صفر قيم محسوبة** («أيّ الألوان يمكن أن تعطينا 3 كرات من نفس اللون؟ ولماذا؟») يُبثّ
  على أول طلب إجراء/مساعدة (`_has_prior_tutoring_step` False) وعلى «كيف افهم للتمرين»
  (`_is_first_help_request` — كانت تسقط للـ LLM fallback العام)؛ الطالب يولّد بصيرة
  «البيضاء مستحيلة» بنفسه، وإجابة الألوان تُتحقَّق بالإشارات اللونية **القائمة**.

### القواعد الدائمة (D-155 — لا تُكسر بدون ADR)
1. **إجابة الطالب تُحكم بالمحرك الرمزي** (أرقام combo) قبل أي LLM؛ الصحيحة تُعترف
   **صراحةً** وتُقدِّم — ممنوع إعادة سؤال ما أُجيب للتو.
2. **السؤال المعلّق يُشتق من قوالبنا بلا أرقام**: أي قالب سؤال جديد في البُناة يُسجَّل في
   `_STEP_QUESTION_MARKERS` وإلا صار سؤاله غير مرئي للتحقّق.
3. **سؤال مفاهيمي/استفهامي أثناء الحوار السقراطي ⇒ الشرح المفاهيمي (D-125)**، ليس
   إجابةً تُقيَّم. «هل» تبقى إجابة (تطلب تأكيداً).
4. **التشخيص قبل المحتوى**: أول طلب مساعدة ⇒ probe واحد بلا قيم محسوبة، ثم انتظار المحاولة.
5. **الـ reveal آخر البدائل دائماً** — «الخطوة التالية غير المُجابة» أولاً.
6. **تأكيد ما ولّده الطالب ليس كشفاً** — لكن نصّ التأكيد الحتمي يبقى بلا أرقام (يَنجو من
   حجب D-113 بنيوياً).

### التحقق (2026-07-02)
- **Sandbox** (pydantic/Postgres محجوبان — §6.55): stub-harness `verify_d155.py` **يعيد
  الترانسكريبت الحرفي — ALL PASS**: «كيف احل السؤال الأول» ⇒ probe (صفر حساب)؛ إجابة
  الألوان ⇒ اعتراف + خطوة البسط؛ «نفس الشئ مع 11» ⇒ direction ⇒ خطوة النسبة (لا reveal
  superset)؛ «هل هي 14 من 165» ⇒ **تأكيد + سؤال الحادثة B**؛ المفاهيمي يهرب للـ D-125؛
  صفر near-dup عبر الأدوار؛ كل النصوص الجديدة تنجو من `redact_final_answers` الحقيقية.
  بوّابة `check_pedagogical_os` **15 فحصاً** (منها 3 جديدة D-155 + probe-first) ✅ | ruff
  check/format ✅ | py_compile 3.12 ✅ | runtime_truth --check ✅ | test-hygiene ✅ |
  17 اختبار node ✅. اختبارات جديدة: `test_iss122_acknowledge_and_advance.py` (24)؛
  مواءمة `test_iss121` لعقد السُّلّم المُعاد ترتيبه (D-105: مواءمة لا تعطيل).
- **Codespaces (E2E حيّ إلزامي)**: إعادة الترانسكريبت عبر المتصفح بالدخولين الحقيقيين —
  التشخيص أولاً، الاعتراف بالإجابة الصحيحة والتقدّم للحادثة B، شرح العلاقة عند
  «لماذا/العلاقة»، صفر تكرار.
...
## 6.132 محرّك الإتقان الصادق — إغلاق حلقة «فجوة الوهم» (المرحلة A) (2026-07-04, ISS-123 / D-157)

> **النجم القطبي مكسور:** المنصّة تعلن أن مقياس النجاح الوحيد هو **فجوة الوهم =
> الأداء المدعوم − الإتقان الدائم** (§0.6)، لكن التحليل الحي أثبت أن `illusion_gap ≈
> assisted` دائماً — الجانب «الصادق» (durable) لا يتحرّك. المنصّة تقيس *الوهم* ولا
> تستطيع قياس *الصدق*. هذا القسم يحكم إصلاح المقياس — لا يُكسر بدون ADR.

### الجذور الثلاثة (ISS-123 — file:line)
1. **إشارة الصواب تفترض الخطأ افتراضياً**: `infer_correctness_signal:263` تُرجِع `False`
   لأي دور بلا عبارة إتقان صريحة ⇒ كل رسالة محايدة (طلب تمرين/تحية) «إجابة خاطئة»
   تُهبِط الإتقان بلا دليل وتُسمِّم القناتين.
2. **القناة الدائمة مُجمَّدة على 0.0**: البوّابة الثنائية الصارمة
   (`correct ∧ support≥5 ∧ delay≥24h ∧ novel`) لا تتحقّق في الدردشة؛ و`_bkt_task` كان
   يُطلَق قبل حساب `support_level` ⇒ durable يُحمَل دون تغيير.
3. **لا بثّ للمقياس (M9 مفقود)**: `tutor_metrics` بلا أي عدّاد لفجوة الوهم/القناتين.

### الإصلاح (D-157 — المرحلة A، حتمي 100%، صفر LLM، fail-open)
- **A1 — إشارة صواب ثلاثية الحالة** (`infer_correctness_signal_3state` +
  `update_mastery_3state`): CORRECT/INCORRECT/UNKNOWN. UNKNOWN ⇒ يُحمَل prior دون تغيير.
- **A1b — تجاوز رمزي مُتحقَّق** (`customer_chat._derive_correctness_override`): الأوراكل
  الحتمي (D-155) يحكم إشارة الصواب لأدوار الاحتمالات (عالي الدقة: يؤكّد الصواب فقط).
- **A2 — إصلاح ترتيب الإطلاق**: `_bkt_task` نُقل إلى ما بعد حساب `support_level` (كان
  يتجمّد durable) — يزيل أيضاً سباق read/write. `novel_item` مشتقّ من `prior_count == 0`.
- **A3 — منحنى نسيان متّصل** (`durable_update_continuous` + `half_life_days` +
  `predicted_recall`): durable قوّةٌ تضمحلّ بالزمن (`2^(−Δt/نصف-العمر)`) وتتراكم بالدليل
  غير المدعوم. **الثابت المضاد للوهم:** الكسب ∝ `generation_weight(support)` ⇒ ≈0 عند
  الدعم الثقيل (support=1 يمنح ~عُشر كسب support=5). يَعكِس البوّابة الثنائية.
- **A4 — بثّ M9**: `record_illusion_gap`/`record_mastery_channels`/`record_durable_rise`
  ⇒ `cogniforge_tutor_illusion_gap` + `_assisted_mastery` + `_durable_mastery` +
  `_durable_rise_total{cause}` من `bkt_persistence.evaluate_and_record`. لوحة
  `observability/grafana/dashboards/180-illusion-gap.json` (النجم القطبي).

### القواعد الـ 5 الدائمة (D-157 — لا تُكسر بدون ADR)
1. **إشارة الصواب ثلاثية**: UNKNOWN (لا دليل) لا يُحدِّث الإتقان أبداً — لا افتراض خطأ.
2. **الثابت المضاد للوهم**: durable لا يرتفع من دليل مدعوم — الكسب ∝ `generation_weight`
   فيقارب الصفر عند الدعم الثقيل. لا تحذف هذا التناسب.
3. **الأرقام/الصحّة من المحرك الرمزي حين توفّر** (heuristics → exact، D-155).
4. **فجوة الوهم تُبَثّ إلى Prometheus** (M9) — كل مقياس له مُصدِر حيّ (§6.21، لا زومبي).
5. **durable لا يرتفع بصدق إلا بدليل غير مدعوم مؤجَّل** — آلية توليده (وضع تحقّق M8 +
   مُجدوِل مراجعة + بوّابة M6) هي المرحلة B؛ داخل جلسة واحدة يبقى durable منخفضاً (صحيح).

### التحقق (2026-07-04)
- **Sandbox** (pydantic/Postgres/Prometheus محجوبة — §6.55): 32 اختبار standalone في
  `tests/services/test_d126_two_signal_bkt.py` — البرهان الحاسم (مُسلَّم الحل
  durable=0.044 مقابل مُولِّد 0.746) + الثابت المضاد للوهم + الإشارة الثلاثية + منحنى
  النسيان + توصيل M9 — ALL PASS. `ruff check/format` + `runtime_truth --check` ✅.
- **Codespaces (E2E حيّ إلزامي)**: `bash .devcontainer/supervisor.sh` + WS بالدخولين
  الحقيقيين (`houssamannaba963@gmail.com` / admin `benmerahhoussam16@gmail.com`) مقابل
  OpenRouter + Supabase: durable يرتفع على دليل غير مدعوم مؤجَّل، `illusion_gap` يُبَثّ،
  لوحة 180 تُملأ. جسر `scripts/db_bridge.py` (HTTPS:443) للتحقق من صفوف
  `student_bkt_analytics` حين تُحجب منافذ Postgres. `check_skills_doctrine` +
  `check_pedagogical_os` (تحتاج pydantic) تُشغَّل في CI.

### السلسلة (D-156 → D-157)
| Decision | الموضوع |
|----------|---------|
| D-156 | تنظيم التوثيق الموحَّد + بوّابة الأرشفة |
| **D-157** | **محرّك الإتقان الصادق (المرحلة A): إشارة ثلاثية + منحنى نسيان متّصل (durable يرتفع بصدق) + تجاوز رمزي (D-155) + إصلاح ترتيب الإطلاق + بثّ M9 + لوحة النجم القطبي — يُصلِح ISS-123 (المقياس الوحيد للنجاح كان مكسوراً بنيوياً)** |

---

## 6.133 محرّك الدور المعرفي — tutor_state مصدراً وحيداً لقرار الدور + إصلاح المخطط الميت (2026-07-05, ISS-124 / D-158)

> **الكارثة الحيّة تنجو من سلسلة كاملة من الإصلاحات (D-113→D-155):** على تمرين الاحتمالات
> BAC-2024 ما زال المعلّم (S1) يقول «إجابتك في الطريق الصحيح —» ثم **يُفرّغ الحل** (البسط 14 +
> المقام 165)؛ (S2) حين يُجيب الطالب «14 على 165» **صحيحاً** ⇒ إعادة تفريغ لا اعتراف؛ (S3) «كيف
> نحسب الحادثة A» ⇒ تفريغ فوري لا سؤال تشخيصي. أطّر المالك العلاج بـ **«هياكل البيانات كنموذج
> معرفي»** (Graph / FSM / حالة علائقية O(1)). القاعدة الحاكمة: **لا يُكسر بدون ADR.**

### الجذر الأعمق (ISS-124 — مؤكَّد، بعضه حيّاً عبر جسر Supabase HTTPS)
السبب أن الكارثة نجت من كل ترقيع سابق: **هياكل البيانات التي تُفترض أنها تُمثِّل «عقل الطالب» إمّا
مزيفة أو ميتة.**
1. **`tutor_state.kc_progress` يُحمَّل ولا يُكتَب أبداً** — `record_turn` (`app/services/analytics/tutor_state_service.py:99`)
   بلا معامل `kc_progress` ⇒ المصدر الوحيد للحقيقة `{}` دائماً ⇒ كل مهارة ترجع لمسح نصّ السجل الهشّ.
2. **المخطط ميت على الإنتاج** — 7 أعمدة D-144 (`learning_stage`…`next_best_action`) مفقودة لأن ALTERs
   كانت مُفلترة خطأً تحت مدخل `bac_exercise_questions` (لا `tutor_state`) في `app/core/db_schema_config.py`،
   و`create_table` بـ `CREATE TABLE IF NOT EXISTS` (no-op على جدول D-142 القائم). النتيجة: **كل كتابة
   tutor_state تفشل صامتة** (حيّاً: صفّان فقط، آخر كتابة 2026-06-26 — كل معمارية D-142→D-157 لم تُخزَّن قط).
3. **القرار مُجزّأ** عبر ~13 كتلة `yield…return` في `orchestrator_client.chat_with_agent`؛ كلٌّ يُعيد بناء
   حالته من النصّ وحارس تكراره لا يرى الكتل الأخرى.
4. **S1** `_build_symbolic_reveal:3097` يُفرّغ البسط+المقام بالتصميم. **S2** الاعتراف موجود
   (`_verify_numeric_answer:2414`→`final_ratio`؛ «أحسنت ✅») لكنه سجين خلف `_in_socratic_dialogue:3170`
   الذي يرفض الدور حين تتجاوز رسالة المساعد السابقة **600 حرف** — وتفريغ S1 نفسه يتجاوزها (النظام يُعمي
   نفسه عن إجابة الطالب الصحيحة). **S3** probe التشخيص غير قابل للوصول (علامات هشّة + `understanding_state` يسبقه).
5. علم `_semantic_tutor_enabled` مُعرَّف مرّتين بافتراضين متعارضين؛ وكتلة PolicyEngine مكرَّرة ميتة في `except`.

### الإصلاح (D-158 — Phase 1، إضافي فوق الكتل القائمة، حتمي، صفر LLM، fail-open)
- **علم موحَّد** (`app/core/feature_flags.py`): `semantic_tutor_enabled()` افتراض True + `cognitive_turn_enabled()`
  افتراض False؛ التعريفان المتعارضان يُفوّضان إليه.
- **`kc_progress` يُكتَب أخيراً** (الشوكة): `record_turn(kc_progress=...)` يدمج مفتاحاً-مفتاحاً؛ `customer_chat`
  يمرّر الدلتا التي يبنيها `_cognitive_turn` عبر dict `tutor_state` المشترك.
- **إصلاح المخطط الميت**: ALTERs الـ7 نُقلت إلى مدخل `tutor_state.auto_fix` الصحيح (+ `kc_progress` دفاعياً)؛
  طُبِّقت **حيّاً على DB الإنتاج عبر الجسر** (12→19 عموداً — أول مرة يقبل الجدول الكتابة).
- **`_cognitive_turn`** (طبقة القرار الموحَّدة فوق `kc_progress` الدائم، خلف `COGNITIVE_TURN_ENABLED`): **S2**
  اعتراف+تقدّم على المستوى الأعلى (لا يمرّ عبر `_in_socratic_dialogue` ⇒ سجن 600-حرف مكسور)؛ **S3** تشخيص
  عند `attempts==0` (مقاد بالحالة الدائمة لا بقوائم علامات)؛ **S1** خطوة **واحدة** تدريجية من
  `representations_delivered` (لا تفريغ، لا تكرار عبر الكتل بنيوياً). `_build_symbolic_step`/`_build_diagnostic_probe`/
  `_verify_numeric_answer` مُعاد استخدامها (صفر LLM).
- **`_build_symbolic_reveal(delivered=...)`**: يحذف الكتلة المعروضة مسبقاً؛ الافتراضي (None) مطابق بايتياً.
- **حذف الكتلة الميتة** في `except` التحية.

### القواعد الـ7 الدائمة (D-158 — لا تُكسر بدون ADR)
1. **`tutor_state.kc_progress` هو المصدر الوحيد** لِـ `representations_delivered` / `_pending` — لا مسح نصّ
   السجل لإعادة بناء المستوى/المفهوم النشط.
2. **خطوة في `representations_delivered` لا تُعاد أبداً** (منع تكرار عبر الكتل، لا داخل كتلة واحدة فقط).
3. **الإجابة الرقمية الصحيحة تُعترَف على المستوى الأعلى** بلا شرط طول الرسالة السابقة (سجن 600-حرف ممنوع عودته).
4. **التشخيص قبل الشرح مقاد بـ `attempts` الدائم** لا بقوائم علامات هشّة («كيف نحسب» = «كيف افهم»).
5. **أي عمود يكتبه ORM يجب أن يملك `auto_fix` تحت مدخل جدوله نفسه** في `db_schema_config.py` (وإلا يُكسَر
   صامتاً على أي نشر نظيف — الجذر الأعمق هنا).
6. **`COGNITIVE_TURN_ENABLED` افتراض OFF** ⇒ سلوك اليوم دون تغيير؛ التفعيل بعد التحقّق الحيّ في Codespaces.
7. **fail-open مطلق**: `_cognitive_turn` داخل `try/except` ولا يُجهض دور الطالب أبداً.

### التحقق (2026-07-05)
- **Sandbox (pip/Postgres محجوبان — §6.55):** إصلاح المخطط طُبِّق وتُحقِّق حيّاً عبر الجسر (tutor_state
  12→19 عموداً على Supabase الإنتاج). **إعادة تشغيل الترانسكليت الكارثي على مصدر الدوال الفعلي**
  (`scripts/verify_d158_live.py` — `ast` يقتطع المصدر ويُنفِّذه مقابل combo BAC-2024 حقيقي): Turn1 «كيف
  نحسب» → سؤال تشخيصي «أيّ الألوان…» (لا تفريغ)؛ Turn2 → خطوة البسط فقط `C(4,3)=4·C(5,3)=10·4+10=14` (لا
  المقام)؛ Turn3 «14 على 165» بعد رسالة سابقة **1536 حرفاً** → «أحسنت ✅ … الحادثة B» (سجن 600-حرف مكسور)؛
  صفر خطوة مُكرَّرة — **6/6 PASS**. `check_pedagogical_os` **15/15 ✅** + `py_compile` + `ruff`.
- **Codespaces (E2E حيّ إلزامي):** `bash .devcontainer/supervisor.sh` (auto_fix يُطبّق على أي DB) + WS
  بالدخولين الحقيقيين (`houssamannaba963@gmail.com` / admin `benmerahhoussam16@gmail.com`) + OpenRouter +
  Supabase + `COGNITIVE_TURN_ENABLED=1` عبر `scripts/verify_d158_e2e.py`: لا تفريغ، «14 على 165» تُعترَف لا
  تُعاد، «كيف نحسب» ⇒ سؤال تشخيصي؛ فحص صفّ `tutor_state` (`kc_progress.representations_delivered` ينمو،
  `_pending` يُمسَح على الإجابة الصحيحة). `check_skills_doctrine` يُشغَّل في CI (يحتاج pydantic).

### Phase 2 (تصميم مؤجَّل — لم يُبنَ الآن) — ترقية الهياكل المزيفة إلى ثورية حقيقية
الآن وقد صار `tutor_state` مصدراً حيّاً للحقيقة: **(1)** ترقية `MISCONCEPTION_GRAPH` (dict مسطّح 7 عُقد بلا
حواف) إلى **graph حقيقي قابل للاجتياز** بربط جدولَي `knowledge_nodes`+`knowledge_edges` القائمين (تشخيص
الجذر عبر الحواف — أساس «فجوة الوهم»). **(2)** استبدال «FSM» التصعيد (يُعيد بناء المستوى من مسح 22-حرفاً
للتاريخ) بـ **حقل حالة FSM دائم** في `tutor_state` (يتطلب `auto_fix` migration). **(3)** تقاعد إعادة بناء
النصّ في `understanding_state` لقراءة `kc_progress` الدائم. **(4) تطوير طبقة قواعد البيانات/ORM للمستقبل
الخارق:** فهرسة `tutor_state(conversation_id)` (فريد، O(1) موجود) + فهارس زمنية على `student_bkt_analytics`
(append-only، D-074)، مخطّط `kc_progress` مُهيكَل (Pydantic بدل JSON حرّ)، وتقييم متجر متجهات/NoSQL
لبنك التمارين الملياري — كلٌّ خلف علم + بوّابة قياس (§0.6: لا توسيع قبل إثبات اللِّفت). القاعدة: **صفر LLM
في مسار الأرقام، حتمي قابل لـ pytest، fail-open** يبقى ساري المفعول عبر كل مرحلة.

### السلسلة (D-157 → D-158)
| Decision | الموضوع |
|----------|---------|
| D-157 | محرّك الإتقان الصادق (المرحلة A) — إغلاق حلقة فجوة الوهم |
| **D-158** | **محرّك الدور المعرفي: `tutor_state.kc_progress` مصدراً وحيداً لقرار الدور + إصلاح المخطط الميت (12→19 عموداً) + `_cognitive_turn` (S1/S2/S3 مقتولة بنيوياً) — يُصلِح ISS-124** |

---

## 6.134 الهياكل المعرفية الحقيقية — D-158 Phase 2 مُنفَّذة + المحرّك المعرفي مُفعَّل افتراضياً (2026-07-07, ISS-125 / D-159)

> **فلسفة المالك المُنفَّذة حرفياً:** «الخوارزمية بلا هيكل بيانات صحيح = محرك فيراري على دراجة
> خشبية». هذا القسم يحوّل الهياكل «المزيفة» الأربعة (JSON حرّ / FSM من مسح نصّ / dict مسطّح بلا
> حواف / جداول ZOMBIE) إلى هياكل معرفية حقيقية — ويُفعّل محرّك الدور المعرفي (D-158) افتراضياً
> بعد نجاح E2E الحي الكامل. لا يُكسر بدون ADR.

### الترقيات الخمس (كلها حتمية، صفر LLM في الأرقام، fail-open)

| الهيكل | قبل D-159 | بعد D-159 |
|--------|-----------|-----------|
| `kc_progress` | JSON حرّ + dict literals يدوية | **مخطط مُهيكَل** `kc_progress_schema.py` (KCEntry stdlib dataclass + parse fail-open + توافق خلفي كامل) |
| FSM التصعيد | إعادة بناء من مسح نصّ التاريخ (يتبخر خارج نافذة الـ50 وعند إعادة التشغيل) | **رُتب دائمة** `escalation_levels` في tutor_state (اتحاد الدائم + النصّ كشبكة أمان) — C1 المُثبَت: نافذة فارغة + [L1] دائم ⇒ L2 لا تكرار |
| `understanding_state` | إعادة بناء من نصّ رسائل المساعد (أعمى عن حجب D-113) | **الحالة الدائمة سلطة** (ترقية monotone) + `understood_kc_id` يُكتب فوراً في الدلتا |
| `MISCONCEPTION_GRAPH` | dict مسطّح بلا حواف | **graph حقيقي**: `GraphEdge` + 12 حافة + `diagnose_root` (DFS يجد أعمق شرط مسبق ضعيف بحسب الحالة الدائمة ⇒ التدخّل يستهدف الجذر لا العرَض) |
| `knowledge_nodes`/`knowledge_edges` | ZOMBIE (صفر مستهلك) | **مرآة حيّة**: `scripts/sync_knowledge_graph.py` (UUIDv5 idempotent + بوّابة تكافؤ) — **نُفِّذت حيّاً على Supabase الإنتاجي: 21 عقدة + 19 حافة، parity MATCH** |
| `_cognitive_turn` | عقدة واحدة مُجمَّدة (A) تعِد بـ B ثم تنساها | **متعدد العُقد**: إتقان A يبذر عقدة B (`_KC_PROB_B` + `_cognitive_turn_event_b` + `_load_canonical_parity`/D-143) — آلة حالات معرفية حقيقية |

### التفعيل (قرار المالك — قاعدة D-158-f استوفيت)
`COGNITIVE_TURN_ENABLED` افتراضه **True** في الموضعين (`feature_flags.py` + `AppSettings`).
**ISS-125 (مُكتشَف حيّاً):** افتراض settings القديم (False) كان يطغى على قارئ العلم حين يغيب env
(نفس عائلة جذر ISS-124 #4) ⇒ القاعدة الدائمة: أي علم يُعرَّف في الموضعين **بنفس القيمة**،
والاختبار `test_settings_default_matches_flag` يحرسه. rollback فوري: `COGNITIVE_TURN_ENABLED=0`.

### التحقق الحي (2026-07-07 — E2E FULL-STACK حقيقي)
- **pip يعمل في هذا الـ sandbox** (خلافاً لنمط §6.55 التاريخي) ⇒ E2E كامل جرى هنا:
  uvicorn حقيقي + WS + JWT + تسجيل دخول المستخدم الحقيقي + OpenRouter مفتاح حقيقي + SQLite
  (Postgres TCP يبقى محجوباً؛ Supabase الإنتاجي عبر جسر HTTPS).
- `scripts/verify_d159_live.py` (الكود الحقيقي): **25/25**. `scripts/verify_d159_e2e.py`
  (WS كامل): U1 تمرين → U2 probe → U3 «14 على 165» ⇒ أحسنت + الانتقال لـ B → U4 «الفردية 8»
  ⇒ خطوة النسبة → U5 «56 من 165» ⇒ إقفال B؛ وقراءة DB: `prob_event_a=understood` +
  `prob_event_b=understood` — **مرّتين: بالعلم صراحةً ثم بالافتراضات الحقيقية (default-on)**.
- Supabase الإنتاجي: مزامنة الـ graph + اجتياز `prerequisite_of` حي بـ JOIN عبر الجسر.
- بوّابات: `check_pedagogical_os` **19/19** (4 فحوص D-159: structured-kc/persistent-FSM/
  graph-edges/multi-KC) + doctrine + hygiene + structure + guardrails + runtime_truth + ruff.
- doctrine bumps: SEMANTIC_PROPERTY **1.4.0** · PEDAGOGICAL_ESCALATION **1.2.0** ·
  UNDERSTANDING_STATE **1.1.0**.

### القواعد الدائمة (D-159 — لا تُكسر بدون ADR)
1. **كل قراءة/كتابة لمدخل `kc_progress` عبر `kc_progress_schema`** — ممنوع dict literals يدوية
   في مسارات القرار.
2. **ذاكرة التصعيد سلطتها الحالة الدائمة** (`delivered_levels`)؛ مسح النصّ شبكة أمان — الاتحاد
   يمنع تكرار الرُّتبة من أي مصدر؛ وكل رُتبة تُبثّ تُكتب فوراً في الدلتا.
3. **الحالة الدائمة سلطة `understanding_state`** والترقية أحادية الاتجاه (لا تُنزِل ما أثبته النصّ).
4. **الحواف data لا كود**؛ `diagnose_root` لا يتدخّل بلا سجل دائم (لا تدخّل أعمى)؛ الجدولان
   مرآة tooling — **المسار الحي يبقى in-code حتمياً** (لا I/O لقاعدة البيانات في مسار الدور).
5. **أي علم feature يُعرَّف افتراضه في الموضعين بنفس القيمة** (feature_flags + AppSettings).
6. **تعميم المحرّك للحوادث C/D/X/E(X) وتوسيع الحواف للمنهج** مؤجَّل خلف بوّابة «القياس قبل
   التوسّع» (§0.6) — بعد خُضرة مقاييس A→B حيّاً.

### السلسلة (D-158 → D-159)
| Decision | الموضوع |
|----------|---------|
| D-158 | محرّك الدور المعرفي (Phase 1 — خلف علم OFF) |
| **D-159** | **الهياكل المعرفية الحقيقية (Phase 2): مخطط مُهيكَل + FSM دائم + graph بحواف + مرآة DB حيّة + محرّك متعدد العُقد (A→B) + التفعيل الافتراضي (E2E حي كامل) — يُصلِح ISS-125** |

---

## 6.135 قتل كارثة تدريس الاحتمالات من الجذر — «اشرح اشتقاق هذه الخطوة» (2026-07-08, ISS-126 / D-160)

> **الكارثة (transcript حيّ، post-D-159):** الطالب يسأل «كيف حسبنا 4» (كيف اشتُقّت `C(4,3)=4`) ثلاث
> مرّات ويتلقّى **النصّ نفسه حرفياً** («لنُكمل معاً… ركّب الاحتمال بنفسك… فما قيمة P(A)؟»). المرّة
> الـ47 لنمط الفشل عبر D-113→D-159. القرار (المالك): **جذري + دمج محدود** — لا كاشف رقم 47.
...
1. **نيّة مفقودة**: `_detect_subpart_question` يربط الألوان و14/165 فقط — لا القيم الملائمة 4/10 ⇒
   «كيف حسبنا 4» يسقط للإنقاذ النهائي المكرَّر.
2. **تكرار غير محروس**: الإنقاذ النهائي `_cognitive_turn → _build_symbolic_reveal` كان يُرجَع بلا
   `_recently_emitted` (حلقة S1 محروسة، الإنقاذ لا).
...
- **F1** — حارس تكرار شامل: الإنقاذ النهائي وكل فرع في `_cognitive_turn` يمرّ عبر `_recently_emitted`؛
  عند التكرار يُصعّد للاشتقاق المُعلِّم لا يُعيد.
- **F2** — `_detect_step_explanation(question, combo)` **data-driven** (يطابق أرقام السؤال ضد
  ...
  مشروط بعلامة شرح `_STEP_EXPLAIN_MARKERS`) + مُعامل `forced_subpart` في
  `_build_probability_direct_explanation` (سطر واحد — يُعيد استخدام كوده التعليمي، صفر كود جديد).
  موضوع **قبل** S2 ⇒ طلب الشرح الصريح يهزم تحقّق الإجابة الرقمية.
...
- **F4** — نفس النيّة+الحارس في الكتلة القديمة (escape-hatch، defense-in-depth) + الدمج البنيوي:
  `_cognitive_turn` يُرجِع مبكراً ⇒ الكتل الاحتمالية القديمة مسارات ميتة للتدفّق الطبيعي. **لا حذف**
  (خطر CI) — مؤجَّل لـ M13.

### القواعد الخمس الدائمة (D-160 — لا تُكسر بدون ADR)
1. **لا فرع في `_cognitive_turn` يُعيد نصّاً** — `_recently_emitted` قبل كل بثّ.
2. **طلب الشرح الصريح (بعلامة) يُعلّم دائماً** — يهزم تحقّق الإجابة الرقمية (F2 قبل S2). «اشرح كيف
   حسبنا 14» شرحٌ لا إجابةَ «14»؛ الإجابة بلا علامة تبقى لـ S2.
3. **الكشف data-driven من قيم combo** — لا 4/10 مُصلَّبة (يعمل لأي تمرين — أساس ملايير التمارين).
4. **الطالب العالق يتعلّم اشتقاقاً ملموساً** — لا كشف للنسبة النهائية (M6/M8).
5. **`_build_probability_direct_explanation` هو المصدر التعليمي الوحيد** (عبر `forced_subpart`) —
   لا كود تعليمي مُكرَّر.

### التحقق (2026-07-08)
- **Sandbox** (يحجب pip/Postgres — نمط §6.55): stub-harness (الكود الحقيقي المُقتطَع بـ ast +
  `_verify_numeric_answer` الحقيقي) — «كيف حسبنا 4» ×3 ⇒ TEACH:red/reveal/TEACH:green (صفر تكرار)؛
  «اشرح كيف حسبنا 14» ⇒ sum (لا اختطاف)؛ «14»/«14 على 165» بلا علامة ⇒ S2 سليم. اختبار جديد
  `tests/services/test_iss126_step_explanation.py` (source-guards deps-free + detector + behavioral).
  `check_pedagogical_os` (+قاعدة D-160 `check_step_explanation_teaches`) · ruff · runtime_truth — خضراء.
- **Codespaces (E2E حيّ إلزامي)**: `bash .devcontainer/supervisor.sh` + WS بالدخولين الحقيقيين
  (`houssamannaba963@gmail.com` / admin `benmerahhoussam16@gmail.com`) مقابل OpenRouter + Supabase:
  «كيف حسبنا 4» ⇒ اشتقاق الحمراء، «كيف حسبنا 10» ⇒ الخضراء، صفر تكرار حرفي، الطالب العالق يتقدّم.
  فحص `tutor_state.kc_progress.representations_delivered` ينمو عبر جسر Supabase HTTPS.
...
## 6.137 «السؤال ليس إجابة» — بوّابة الفعل الكلامي + تصحيح السؤال المعلّق + التوافيق (2026-07-10, ISS-128 / D-162)

> **الكارثة (transcript حيّ، BAC-2024):** الطالب سأل سؤالاً مفاهيمياً «**ماذا نسمي** حساب 4
> و 10 لم افهمها؟» (يطلب **اسم** العملية = التوافيق C(n,k))، فردّ النظام «**إجابتك في
> الطريق الصحيح** —» وكأنها إجابة صحيحة، ثم **كشف جواب سؤاله المعلّق بنفسه** (C(11,3)=165)
> وتقدّم للنسبة — تجاهل تام للسؤال. تحذير المالك: «لو أضفنا ملايير التمارين سيتدمّر
> المشروع» ⇒ الإصلاح بنيوي data-driven لا ترقيع صياغة. لا يُكسر بدون ADR.

### الجذور الأربعة (مؤكَّدة file:line)
1. **`_verify_numeric_answer` بلا حارس استفهامي** — يستخرج {4,10} من *سؤال* ويعتبره إجابة.
2. **تسمية pending خاطئة**: خطوة البسط خزّنت `new_pending="numerator"` بينما سؤالها المطروح
   يسأل عن **المقام** («كم عدد كل الطرق الممكنة؟») ⇒ `favs <= nums` ({4,10} ⊆ {4,10})
   أطلق `step_correct` زائفاً وكشف 165.
3. **نية التسمية عمياء في كل الطبقات**: «نسمي/ما اسم» غائبة عن `_STEP_EXPLAIN_MARKERS` +
   `_DEFINITIONAL_MARKERS` + `_detect_conceptual_question` + `_QUESTION_OPENERS_NOT_ANSWERS`
   (لم تكن تحوي حتى «ماذا»)؛ ولا مفهوم `combinations` في `PROPERTY_REGISTRY`.
4. **`_cognitive_turn` يسبق كل البوّابات المفاهيمية** (D-125/D-132) — لا هروب للسؤال.

### الإصلاح (حتمي 100%، صفر LLM في الأرقام، fail-open، data-driven من combo)
- **بوّابة الفعل الكلامي** `_is_question_not_answer`: بادئات استفهامية موسَّعة + علامات
  تسمية `_NAMING_MARKERS`؛ «هل» مستثناة عمداً (D-155). موصولة **أول** `_verify_numeric_answer`
  (تحمي كل مواقع الاستدعاء) + بوّابة قبل سُلّم S1 (السؤال وسط الحوار يهرب لطبقات الإجابة؛
  «كيف/كيفاش» الإجرائية تبقى لسلسلة F2/F1/F3 — D-160).
...
- **مُجيب التسمية الحتمي** (`_detect_naming_question` + `_build_naming_answer`، قبل S2):
  ...
  `PROPERTY_REGISTRY["combinations"]` الجديد) + `_fmt_comb` LaTeX (يَنجو من حجب D-113) +
  **إعادة طرح السؤال المعلّق** مع إبقاء pending (لا تقدّم زائف).
- **Track 2A — تكافؤ split-brain**: العقود مُزامنة إلى port الخدمة المصغرة
  (`probability_tutor.py` — stdlib مستقل، M10-S2.1) + بوّابة CI `check_split_brain_parity`
  (7 عقود في العقلين + «هل» خارج الـ tuples) — أي افتراق مستقبلي يُفشل CI (نمط D-013).
- **المرآة المعرفية**: عقدة `combinations` زُومنت حيّاً إلى Supabase الإنتاجي
  (`sync_knowledge_graph.py` ⇒ 22 عقدة/19 حافة، parity MATCH).

### القواعد الـ5 الدائمة (D-162 — لا تُكسر بدون ADR)
1. **السؤال ليس إجابة أبداً**: أي رسالة استفهامية (عدا «هل») لا تُعترف كإجابة رقمية —
   `_is_question_not_answer` أول `_verify_numeric_answer`.
2. **pending = السؤال المطروح فعلاً**؛ أي قالب سؤال جديد يُسجَّل في `_STEP_QUESTION_MARKERS`.
...
5. **تكافؤ العقلين محروس بـ CI** (`check_split_brain_parity`) — تعديل عقد المعلّم في نسخة
   واحدة دون الأخرى يُفشل البوّابة.

### عقد ملايير التمارين (Track 2C — data-driven بالبناء)
كل كواشف D-162 تقرأ **قيم combo** (المحرّك الرمزي) حصراً — صفر أرقام مُصلَّبة لتمرين بعينه
(اختبار `_other_combo` بأرقام مختلفة كلياً يُثبته). تمرين جديد = صف `bac_exercises` (Supabase،
D-099) + `parsed_entities` + مدخل `KNOWLEDGE_INDEX` للساخن — المحرّك يعمل تلقائياً على أي
تركيبة، لا كود لكل تمرين. **تفكيك God-file** (`orchestrator_client.py`) يبقى M13 (قرار D-160
— نصف قطر الانفجار الاختباري يتعارض مع إلزام CI الأخضر)؛ خطوته المحروسة هنا = بوّابة التكافؤ.

### التحقق الحيّ (2026-07-10 — E2E FULL-STACK حقيقي في الـ sandbox، pip يعمل §6.134)
- **`scripts/verify_d162_e2e.py` — 9/9 PASS**: uvicorn حقيقي + WS + تسجيل دخول المستخدم
  الحقيقي + **OpenRouter بالمفتاح الحقيقي** + SQLite (Postgres TCP محجوب — نمط §6.55):
  T4 الكارثي ⇒ «التوافيق» + إعادة سؤال المقام، **صفر** «إجابتك في الطريق الصحيح»، **صفر**
  كشف 165؛ T5 «165» ⇒ اعتراف + خطوة النسبة؛ T6 «14 على 165» ⇒ «أحسنت ✅» + الحادثة B.
- **Regression حيّ**: `verify_d158_e2e.py` PASS (صفر انحدار في محرّك الدور المعرفي).
- **Supabase الإنتاجي عبر جسر HTTPS**: حيّ (PostgreSQL 17.6) + مزامنة المرآة parity MATCH.
- **اختبارات**: 39 جديدة (`test_iss128_question_speech_act.py`) + 1948 (services/microservices/
  fitness) + 838 (api/contracts/regressions/unit) + 360 مجاورة — **صفر مواءمة لزمت** (الفشل
  الوحيد pre-existing ومستثنى في CI). بوّابات: pedagogical_os (21 فحصاً، منها 2 جديدان) +
  skills_doctrine + runtime_truth + structure + guardrails + test_hygiene + ruff — خضراء.
...
## 6.142 اكتشافات E2E الحي لـ D-168 — سلك نظيف + قاعدة D-102 مفروضة + هوية الإدمن (2026-07-16, ISS-131 / D-169)

> **E2E الحي في Stage E (D-168) كشف كوارث pre-existing** كانت مقنَّعة منذ أسابيع — بالضبط
> ما وُجد الفحص الحي لأجله. كلها أُصلحت بالجذر + اختبارات regression. لا تُكسر بدون ADR.

### الكوارث الخمس المكتشفة (كلها pre-existing، أثبتها الفحص الحي)

| # | الكارثة | الجذر (منذ) | الإصلاح |
|---|---------|-------------|---------|
| 1 | **سؤال عام في محادثة جديدة ⇒ إطار `error`** («ما هي الجاذبية؟» لا يُجاب!) | `context["policy_decision"] = <dataclass>` (D-144) يُسمِّم جسم POST — `TypeError: not JSON serializable` **قبل أي بايت على السلك** ⇒ فشل كل المرشّحين ⇒ ORCHESTRATOR_REQUIRED | `_sanitize_wire_context` (`_INTERNAL_CONTEXT_KEYS = {policy_decision, tutor_state}`) — القرار الداخلي لا يركب السلك أبداً |
| 2 | **الحقن لا يقرأه أحد**: `record_turn` يقرأ من `tutor_state_ctx` لا من الـ context الأعلى | نفس D-144 — كتب في الـ dict الخطأ | الحقن صار في **`tutor_state` المشترك** (مع حفظ هوية الـ dict — لا `or {}` يفكّ المشاركة) |
| 3 | **كل سؤال إدمن يُختطف بالـ probe الاحتمالي** («كم عدد ملفات بايثون؟» ⇒ «لنبدأ من فهمك — نسحب 3 كرات...») | بوّابات سياق الاحتمالات (D-159/D-122) تمسح **كل** الـ history بما فيه رسالة system — برومبت الإدمن (21K حرف) يحوي «كرة/احتمال» — **خرق قاعدة D-102** | فلتر `role in (user, assistant)` في المواقع الثلاثة: `turn_engine._ct_hist` + `chat_turn._history_text` + `probability_ui._combined_text/_history_context` |
| 4 | **دور إدمن يصل السلك ⇒ 403** | معرّف محادثة الإدمن (جدول `admin_conversations`) يُمرَّر لفحص ملكية فضاء العميل (ISS-019 family) | `_wire_conversation_id = None` لنطاق admin — الـ orchestrator يشتق محادثته؛ المونوليث يبقى مالك حفظ رسائل الإدمن (D-006) |
| 5 | **أداة الإدمن تُنفَّذ ثم تُرفض** `ADMIN_ACCESS_DENIED` | `_build_service_jwt` بلا claim إداري و`_is_admin_payload` fail-closed — نفس درس D-162/§6.78 غير مطبَّق على service JWT | `_build_service_jwt(user_id, is_admin=...)` يحمل `is_admin+role` لنطاق admin حصراً (نقل حقيقة مُتحقَّقة عند اتصال WS، لا تصعيد) |

### القواعد الدائمة (D-169 — لا تُكسر بدون ADR)
1. **جسم HTTP نحو الـ orchestrator يمرّ عبر `_sanitize_wire_context`** — أي مفتاح context
   داخلي جديد (كائنات قرار/حالة مونوليث) يُضاف لـ `_INTERNAL_CONTEXT_KEYS`.
2. **قاعدة D-102 شاملة**: أي كاشف/مُستخرِج يقرأ الـ history يُرشِّح
   `role in (user, assistant)` — رسالة system ليست دليلاً من المحادثة أبداً.
3. **الـ handoff الداخلي عبر `tutor_state` المشترك** (هوية dict محفوظة) لا الـ context الأعلى.
4. **معرّفات فضاء الإدمن لا تعبر لفضاء العميل** — نطاق admin يُرسل `conversation_id=None`.
5. **service JWT يحمل هوية القناة بصدق** (admin claim للنطاق الإداري حصراً — أقل صلاحية).

### المتبقي المُشخَّص (ISS-132 — follow-up بـ ADR، ليس انحداراً)
أسئلة **أدوات** الإدمن («كم عدد ملفات بايثون») عبر `/api/chat/messages`: الأداة تُنفَّذ
(`TOOL EXECUTED → 1711 ملف`) لكن `ValidateAccessNode` يرفض لأن `AgentState` (الرسم
الـ13-node) **لا يُصرّح حقول الإدمن** (`is_admin/user_role/scope`) فتسقط عند عبور الرسم،
والـ handler يتجاهل `_jwt_payload`. الفجوة بنيوية منذ D-025 (كانت مقنَّعة بكارثة #1).
الإصلاح يتطلب تغيير مخطط حالة الرسم + تمرير `admin_payload` في الـ handler — ADR مستقل.
**قناة الإدمن نفسها سليمة حياً**: سؤال عام إداري ⇒ إجابة LLM حقيقية عبر الرسم (مُثبَت).

### التحقق الحي (2026-07-16 — بعد الإصلاحات، المكدس الكامل + OpenRouter حقيقي)
- «ما هي الجاذبية الأرضية؟» (محادثة جديدة) ⇒ إجابة 780 حرفاً **عبر الـ orchestrator**
  (POST نجح — كان `error` قبل الإصلاح) ✅ | admin «قانون أوم» ⇒ 230 حرفاً ✅ |
  admin بلا اختطاف probe ✅ | **إعادة كامل حزمة E2E بعد الإصلاحات**: d162 9/9 +
  iss129 (T4 عبر الرسم) + d158 + m10s2 7/7 + stage-c — كلها خضراء ✅.
- Supabase الإنتاجي عبر الجسر (HTTPS:443): PostgreSQL 17.6 + tutor_state 19 عموداً ✅.
- `tests/services/test_iss131_wire_context_and_system_prompt.py` (11 اختباراً).
