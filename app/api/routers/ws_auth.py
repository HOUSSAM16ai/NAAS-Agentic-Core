...
class WsActor:
    """هوية WebSocket مُشتقّة من الـ JWT — بلا استعلام قاعدة بيانات عند الاتصال.

    ISS-100 / D-WS-CONN-001: إنشاء اتصال WebSocket يجب ألا يعتمد على قاعدة
    البيانات. الاعتماد القديم على ``db.get(User)`` عند الاتصال جعل كل اتصال
    رهيناً بـ Supabase؛ تحت الضغط كان يفشل → إغلاق 1013 → إعادة اتصال → تأرجح
    «متصل/غير متصل» في الثواني الأولى ولا إجابة حتى للتحية. الهوية تأتي من الـ
    JWT الموقّع؛ وأي عمل على قاعدة البيانات يحدث لكل دور على حدة مع معالجة أخطائه
    دون إسقاط الاتصال. الحقول المستخدَمة لاحقاً: ``id`` و ``is_admin`` فقط.
    """
