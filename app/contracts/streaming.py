...
KNOWN_UI_COMPONENTS: frozenset[str] = frozenset(
    {
        ...
        "learning_path_card",
        # D-080/D-191: بطاقة شرح الرياضيات — تُبنى **حتمياً** في `enrich_node`
        # (العقدة الرابعة من خط الرياضيات، بلا LLM). كانت مُسجَّلة في واجهة
        # `GenerativeUIRenderer` وغائبة عن هذه القائمة، فكان المُطبِّع يرفضها
        # ولا تصل الطالب أبداً — عقدٌ مُعلَن بنصفه. تحرس D-097 الوجه الآخر:
        # ممنوع بناء هذه البطاقة من **نثر LLM حرّ** (`_try_build_math_ui_component`
        # مُعطَّلة دائماً لهذا السبب) — المصدر المشروع هو المخرَج المُهيكَل وحده.
