...
class TurnPreemptsCognitiveMixin:
    ...
    async def _stage_escape_hatch(self, ctx: TurnContext) -> AsyncGenerator[dict | str, None]:
        ...
        #
        # ISS-140: **مشروط بسياق الاحتمالات.** «حيرة عامة ⇒ الحل الكامل» قاعدةٌ تخصّ تمرين
        # الاحتمالات وحده — فـ`full_solution` هنا اسمُ مفهومٍ في عقل الاحتمالات لا وصفٌ عامّ.
        # وبلا هذا الشرط كان سؤال فيزياء («اشرح لي قانون أوم») يُعاد إلى `full_solution` بعد
        # أن حيّدته بوّابة الموضوع أعلاه، فيتلقّى الطالب قائمة تشخيص الاحتمالات (بُرهن حيّاً).
        # العتبة `>= 2` تبقى كما هي: عدّاد الحيرة يرى نسخة الدور الحاضر المحفوظة في التاريخ
        # (المونوليث يحفظ قبل بناء الدور — §6.5)، فـ`2` تعني عمليّاً «حيرة واحدة» داخل
        # التمرين — وهو سلوك D-184 المقصود (البؤرة اللاصقة). تصحيح العدّاد وحده يُفقد الطالبَ
        # موضوعَه، فلا يُمَسّ في هذه الجولة (مُسجَّل في ISS-140 كدَين مقصود).
        if (
            _concept == "unknown"
            and _confusion_count >= 2
