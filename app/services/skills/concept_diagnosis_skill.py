...
_EVENT_MARKERS: tuple[str, ...] = (
    ...
    "كيف افهم الحادثة",
    "كيف افهم الحدث",
    # D-131: خصائص الجداء (الحوادث B/C/D) — تُوجَّه لـ event_meaning ثم الطبقة الدلالية.
    "جداء",
    "الجداء",
    "حاصل الضرب",
    "حاصل ضرب",
    ...
    async def _classify_with_llm(cls, question: str) -> Concept | None:
        ...
        try:
            ...
            system_prompt = (
                ...
                "event_meaning (يسأل عن معنى الحادثة A/B/C/D أو خاصية الجداء: فردي/زوجي/معدوم) | "
