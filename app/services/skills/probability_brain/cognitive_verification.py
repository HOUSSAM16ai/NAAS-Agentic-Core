...
class CognitiveVerificationMixin:
    ...
    def _is_prob_context(text: str) -> bool:
        ...
        **D-266 (ISS-159) — نقضٌ قبل الإثبات.** كانت هذه الدالّة مفاتيحَ نصّية عارية،
        وفيها «كرة» التي تعني الكرة الهندسية والفيزيائية أيضاً، فسؤال ميكانيك يصير
        «سياق احتمالات» في **١٣ موضع نداء**. الآن: إشارةُ مادةٍ موجبة غير احتمالية
        **تنقض** السياق قبل أن تُقرأ أيّ مفردة.

        وكان في القائمة `r"\\b165\\b"` و`r"\\b14\\b"` — نصّان خامان داخل `m in t`، أي
        **مطابقة سلسلة** لا تعبير نمطي: البحث كان يجري حرفياً عن `\\b165\\b` فلا يطابق
        أبداً. تعليقُ D-251 يَعِد بحدود كلمات ولم يكن للكود أثر. حُذفا: مفتاحٌ ميت
        يوهم بحمايةٍ غير موجودة، وأرقامُ تمرينٍ بعينه ليست تعريفاً لمادة أصلاً.
        """
        t = text or ""
        try:
