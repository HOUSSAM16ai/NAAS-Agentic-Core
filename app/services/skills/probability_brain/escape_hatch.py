...
class EscapeHatchMixin:
    ...
    def _detect_part_by_name(cls, question: str) -> str | None:
        ...
        ISS-149 — **سؤالٌ عن دورَين يُجاب عن دورَين**. كان الكاشف يمرّ على الأنماط
        بالترتيب ويُرجِع أوّل ما يُطابِق، فسؤال «كيف نفرق بين البسط و المقام؟»
        يُطابِق النمطين معاً ويُرجِع `numerator_method` — لأن `numerator` مكتوبٌ
        أوّلاً في الـtuple، لا لأن الطالب سأل عنه. فتلقّى الطالبُ نصفَ جواب،
        ونصفُ جوابٍ عن سؤال مقارنة ليس نصفَ فائدة بل صفر: المقارنة **هي** السؤال.
        """
        try:
        ...
        if not norm:
            return None
        matched = [part for part, pattern in cls._PART_NAME_PATTERNS if re.search(pattern, norm)]
        if len(matched) > 1:
            return "ratio_method"
    ...
    def _build_probability_direct_explanation(
        ...
        *,
        ...
        try:
            ...
            # ── ISS-149: سُئلنا عن **الفرق** بين الدورين ⇒ نُقابل بينهما ──────
            # «كيف نفرق بين البسط و المقام؟» سؤالُ مقارنة: جوهرُه ما يُميّز أحدهما
            # عن الآخر، لا إجراءُ أحدهما. وتسليمُ «طريقة حساب البسط» وحده كان
            # يترك نصفَ السؤال بلا جواب ثمّ يطلب من الطالب حساب مجموعٍ سُلِّم له
            # أصلاً. المقارنة تُبنى على **الشرط**: المقام يعدّ بلا شرط، والبسط
            # يعدّ ما يستوفي الشرط — ومن هنا كان البسط أصغر دائماً.
            if subpart == "ratio_method":
                _possible_labels = "، ".join(g.label for g in groups if g.is_possible)
                return (
                    f"## البسط والمقام — ما الذي يُفرّق بينهما؟\n\n"
                    f"كلاهما **عدد سحبات**، والفرق بينهما في كلمة واحدة: **الشرط**.\n\n"
                    f"| | المقام | البسط |\n"
                    f"|---|---|---|\n"
                    f"| ماذا يعدّ؟ | كل سحبات الـ{k} كرات | السحبات التي **تحقّق الحادثة** |\n"
                    f"| هل عليه شرط؟ | لا — أي {k} كرات من الكيس | نعم — شرط الحادثة |\n"
                    f"| من أين نعدّ؟ | من الكيس كلّه ({n} كرة) | من كل لونٍ يسمح به الشرط "
                    f"({_possible_labels}) |\n\n"
                    f"ولهذا يكون البسط **جزءاً** من المقام لا شيئاً مستقلاً عنه: "
                    f"كل سحبة تحقّق الحادثة هي أصلاً سحبة ممكنة، والعكس ليس صحيحاً.\n\n"
                    f"سؤالٌ يُثبّت الفرق: لو ألغينا شرط الحادثة تماماً، فماذا يصير "
                    f"البسط في رأيك — ولماذا؟"
                )
