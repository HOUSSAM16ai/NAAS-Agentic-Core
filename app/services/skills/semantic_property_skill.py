...
PROPERTY_REGISTRY: dict[str, PropertySpec] = {
    "same_color": PropertySpec(
        ...
        # D-191 (ISS-140 د): كان هذا التعريف يحمل **تركيبة التمرين المرجعي**
        # حرفياً («كلها بيضاء … عدد البيضاء 2 فقط»)، فطالبٌ كتب كيساً بلا كرات
        # بيضاء كان يقرأ عن كرات لا وجود لها في تمرينه — أرقامٌ لم يذكرها تُطبَع
        # كأنها أرقامه. القاعدة تُصاغ الآن **عامّةً وملموسةً معاً**: تنطبق على أيّ
        # كيس، وتبقى خالية من أيّ عددٍ لم يُعطِه الطالب.
        definition=(
            "الحادثة A هي الحصول على **3 كرات من نفس اللون** عند سحب 3 دفعةً واحدة — "
            "أي أن تكون الكرات الثلاث كلّها من لونٍ واحد مهما كان ذلك اللون. "
            "ولاحظ قاعدةً مفيدة: أيّ لونٍ عددُه في الكيس أقلّ من 3 يستحيل سحب 3 منه، "
            "فلا يُساهم في تحقّق A."
        ),
    ...
    "favorable_cases": PropertySpec(
        ...
        # ISS-149: كانت تحمل الاسم المفرد «البسط» — فسؤال الطالب «كيف نفرق بين
        # البسط و المقام؟» يُطابقه فيُقرأ برهانَ فهمه. اسمُ المفهوم يُذكَر في
        # السؤال عنه بقدر ما يُذكَر في شرحه؛ البرهان **عبارةٌ تصف الآلية**.
        evidence_markers=(
            "البسط هو عدد الحالات",
            "البسط عدد الحالات",
    ...
    "simultaneous_draw": PropertySpec(
        ...
        # ISS-149: «مجموعة» وحدها كلمة عربية شائعة تُطابَق في جملٍ لا علاقة لها
        # بالبرهان — اسمٌ مفرد لا يصلح دليلَ إتقان (نفس صنف «البسط»).
        evidence_markers=(
            "معا في وقت واحد",
            "لا يهم الترتيب",
            "مجموعة لا تسلسل",
    ...
    # D-162 (ISS-128): «ماذا نسمي حساب 4 و 10؟» — الطالب يطلب **اسم** العملية.
    # كان المفهوم موجوداً في enum التشخيص و HIDDEN_GOALS لكنه بلا تعريف هنا ⇒
    # سؤال التسمية كان أعمى في كل الطبقات الحتمية. مدخل data لا فرع كود (D-131).
    "combinations": PropertySpec(
        ...
        title="ماذا نسمي حساب C(n,k)؟ — التوافيق",
        definition=(
        ...
        domain="probability_concept",
        example=(
            "مثال ملموس: عدد طرق اختيار 3 كرات من بين 4 كرات حمراء هو $C_{4}^{3}$ — "
            "نختار «أيّ 3 كرات» دون أن يهمّنا ترتيب سحبها؛ ولهذا نقسم على عدد الترتيبات "
            "الداخلية بدل الاكتفاء بالضرب."
        ),
        evidence_markers=(
            "دون ترتيب",
            "بدون ترتيب",
...
MISCONCEPTION_GRAPH: dict[str, tuple[MisconceptionNode, ...]] = {
    ...
    "same_color": (
        ...
        MisconceptionNode(
            ...
            # D-191 (ISS-140 د): القاعدة عامّة — العدد يقرؤه الطالب من **كيسه**.
            probe="كم كرةً من ذلك اللون في كيسك؟ وكم نحتاج لسحب 3 منه؟",
            intervention=(
                "لونٌ عددُه أقلّ من 3 يستحيل سحب 3 منه، فلا يُحقّق A؛ "
                "لذلك A تتحقّق بالألوان التي عددها 3 فأكثر في كيسك."
            ),
