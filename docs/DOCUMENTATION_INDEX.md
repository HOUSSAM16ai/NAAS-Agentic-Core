...
|---------|--------|------|
| 🏛️ الدستور | `CLAUDE.md` | القوانين التشغيلية الدائمة + سجلّ العقائد §6.x (D-001→**D-261** · آخرها D-261: قشرة تفويض `app/core/database.py` + شرائح `app/core/database_support/`، وD-260: قشرة تفويض `kernel.py` + شرائح `app/core/kernel_support/`). **لا يحمل حالات ولا سرداً مؤرَّخاً** (D-188) |
