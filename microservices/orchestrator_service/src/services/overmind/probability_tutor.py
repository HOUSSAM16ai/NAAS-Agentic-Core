...
#: إشارات الحيرة/الإجراء — **مرآةٌ مُوَرَّدة** من `shared/intent/registry.py`
#: تحرسها `scripts/fitness/check_probability_brain_parity.py` في الاتجاهين (ISS-149).
#:
#: **لماذا مرآة لا استيراد:** دستور الخدمات (القاعدتان 97/98) يمنع مكتبة منطق مشتركة،
#: وهذا الملفّ يُحمَّل بلا حزمة في اختبارات المنفذ فلا يستورد شيئاً. نمط `shared/notation`
#: المُثبَت (D-185) و`response_sanitizer` (D-203).
#:
#: **لماذا اتّسعت:** كانت **٧** علامات مقابل **٢٧** قانونية، بلا بوّابة تكافؤ — وهي
#: تحكم `_is_tutoring_turn`، أي **هل يدخل الدور المسارَ التربوي الحتمي أصلاً أم يسقط
#: إلى الـLLM**. مسبارٌ تفاضلي على العقلين أعطى ٤ انحرافات من ٥ صيغ حيرة واقعية، منها
#: «محتار» و«ماعرفتش» و«je ne comprends pas» — الدارجة والفرنسية على منصّةٍ دستورها
#: يقول «عربي / فرنسي / دارجة». التوريد كان مُصرَّحاً في `_FROZEN_DEBT` بسببٍ منطوق،
#: لكنّ الدستور يشترط أن يُقرَن التوريدُ المقصود ببوّابة تكافؤ (D-206·L5) — ولم تكن.
#:
#: ⚠️ **قيدٌ مُصرَّح:** `_norm` هنا أضعف من `shared.intent.normalize` (لا تطوي الهمزات
#: ولا التشكيل)، ولهذا تُسرَد صيغتا الهمزة معاً («لم افهم» و«لم أفهم»). توحيدُ
#: المُطبِّعَين عملٌ أوسع مُوثَّق في `.memory/issues.md` تحت ISS-149، لا مسكوتٌ عنه.
_CONFUSION = (
    "لم افهم",
    "لم أفهم",
    "لم استوعب",
...
#: ⚠️ `_PROCEDURE` **ليست** مرآةً لنيّة `procedure` القانونية: هنا «كيف»/«كيفاش»
#: المجرّدتان مقصودتان لأن الدور التدريسي يلتقط أيّ سؤال إجرائي مهما كان مفعوله،
#: بينما النيّة القانونية تشترط الفعل («كيف نحسب»). فرقٌ مُصرَّح لا انحراف — ولذلك
#: تحرس البوّابةُ `_CONFUSION` وحدها بتكافؤٍ صارم.
