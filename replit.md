...
**§6.15 — surface the Rebuild action with four click-paths.** The user wanted the "Rebuild Container" button to appear automatically and be clickable directly. VS Code Codespaces doesn't let third-party config inject a real button without shipping an extension, so we surfaced the action through every other path: (1) VS Code's built-in auto-toast on `devcontainer.json` changes; (2) one-terminal-command wrapper `bash .devcontainer/codespace_rebuild.sh` (interactive `gh codespace rebuild`); (3) `.vscode/tasks.json` adds the rebuild as a task in Ctrl+Shift+P → Tasks: Run Task; (4) when Docker is detected as missing AND user is in a Codespace, `on-attach.sh` prints a 16-line ASCII banner listing all four paths inline. None of these affect Replit (gated on `${CODESPACE_NAME}`).
