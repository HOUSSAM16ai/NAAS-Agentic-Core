...
7. `microservices/orchestrator_service/src/services/overmind/graph/search.py` — **Done (D-262 · 2026-08-16):** CodeScene X-Ray job 72 hotspot بدرجة 10/10 (`create_unified_graph` 85 LOC · churn=14 أعلى الملف مطلقًا · `route_intent` churn=5) ⇒ قشرة تفويض نقية 24 LOC (A(2)) + حزمة شرائح نقية `graph/graph_support/` [`_graph.py` تسجيل العقد + الأسلاك الحتمية + compile/checkpointer · `_conditions.py` شرائط route_intent/check_results/check_quality مع DEADLOCK FIX: النيات المجهولة تُقفل إلى `educational` · `_search_shards.py` شرائح البحث الخمس + `_PassthroughNode` · `_sources.py` مانيفست مركّب `GRAPH_SOURCE_FILES`] · كل أسماء الواجهة القديمة أعيد تصديرها (لا كاسر) · radon A(1-3) · 25 اختبارًا جديدًا يثبت مطابقة حرفية (الرسم المجمّع عقدةً عقدة وحافةً حافة) · CI أخضر.
...
11. `app/kernel.py` — **Done (D-260 · 2026-08-15):** CodeScene X-Ray hotspot على النواة ذاتها (272 سطرًا · `_validate_contract_alignment` 35 LOC · churn=2 · تعقيد 10 · Complex Method/Bumpy Road · `_handle_lifespan_events` churn=9) ⇒ قشرة تفويض نقية (كل واجهات kernel.py القديمة أعيد تصديرها بالاسم — صفر كاسر) + حزمة شرائح `app/core/kernel_support/` [`_sources.py` مانيفست مركّب · `lifecycle.py` · `contracts.py` · `otel.py` · `compose.py` نقية بلا حالة] · radon B(8)/B(7)→A · ruff 0.14.0 أخضر · E2E D-259 الحيّ (Supabase + OpenRouter + Tavily MCP + lifespan kernel كامل) أخضر قبل/بعد — صفر تغيير سلوكي.
12. `tests/conftest.py` — **Done (D-258 · 2026-08-15):** CodeScene X-Ray hotspot على بنية الاختبارات (416 سطرًا · `db_lifecycle` 63 LOC · churn=12 · Bumpy Road Ahead · `register_and_login_test_user` 54 LOC) ⇒ قشرة معمارية تفويض نصّية + حزمة شرائح نقية `tests/conftest_support/` (helpers · registry بعزل حقيقي + قفل المحرك ISS-113 · schema مراحل نقية — Bumpy Road مغلق · lifecycle · auth_shards · policy · مانيفست `_sources.py`) — اكتشاف معماري: pytest لا يفعّل autouse لfixtures مستوردة فبقيت قشور التسجيل في الconftest · صفر تغيير سلوكي · الحزمة الكاملة مطابقة · CI أخضر 100%.
13. `app/core/domain/user.py` (مسار الحفظ) — **Done (D-257 · 2026-08-14):** E2E حي على Supabase: أول `save_message` لمسار customer chat يسقط بـ`InvalidRequestError: name 'Mission' is not defined` ⇒ استيراد `Mission` حرفيًا + `foreign_keys=[Mission.initiator_id]` مرجع كائني صفر تغيير سلوكي · رسالة `id=4915` محفوظة فعليًا.
...
| **Pedagogy-engine constitution** | ✅ **D-263** | Owner decision 2026-08-16 (external audit: 8/10 architecture vs 4.5/10 product, 3.5/10 proven feature — "the architecture is not the feature"). Addition, never removal: `.memory/pedagogy_engine_constitution.md` (doctrine + ten non-negotiable laws L1–L10), `scripts/fitness/check_pedagogy_engine.py` wired into `ci.yml:guardrails`, `CLAUDE.md §0.15`, decision D-263 in `.memory/decisions.md` + issues ISS-174→ISS-177; `check_memory_coherence` cap raised 1025→1028 with documented reason. Pre-existing laws (D-153 Pedagogical OS, D-144, D-208, §0.8–§0.14) all remain in force. Live E2E verification (2026-08-16): monolith :8000 + orchestrator :8006 (state_graph) booted against production Supabase with real OpenRouter/Tavily keys — two full WS chat round-trips E2E 6/6 green (`terminal=complete`, `persisted`, `conversation_history` ascending 927→929), the second run a pedagogical probability question answered Socratically (inquiry-first, not a direct answer). Evidence archived: `docs/archive/e2e/D263_E2E_LIVE_2026-08-16.md`. |
| **Adaptive study planner constitution** | ✅ **D-264** | Owner decision 2026-08-16: the paper weekly planner (weekly goals, seven day boxes, notes) becomes an **Adaptive AI Study Planner** — student states goals → system diagnoses them (BKT/FSRS) → deterministic zero-LLM division across the week → completion tracking → replanning from actual performance. Addition, never removal: `.memory/adaptive_study_planner_constitution.md` (doctrine + ten laws L1–L10), `scripts/fitness/check_adaptive_study_planner.py` in `ci.yml:guardrails`, `CLAUDE.md §0.16`, decision D-264 in `.memory/decisions.md` + issues ISS-178→ISS-181. Live E2E verification (2026-08-16): monolith :8000 + orchestrator :8006 against production Supabase — login ×2, full WS turn, `/health`=ok, review queue endpoint live with real OpenRouter/Tavily — second pedagogical probability round answered inquiry-first (evidence archived: `docs/archive/e2e/D264_E2E_LIVE_2026-08-16.md`). |
| **Spec Kit governance constitution** | ✅ **D-265** | Owner decision 2026-08-16: Spec Kit (github.com/spec-kit, MIT, 120k+ stars) adopted as an **execution-governance layer**, not a replacement — plan → design → tasks → spec → implement → prove, enforced above D-153/D-263/D-264 with no law removed. `.memory/spec_kit_governance_constitution.md` (ten laws L1–L10: spec updated with the push, measurable contract not prose, every named enforcer actually exists, proof before acceptance, migration before use, API contract never breaks silently, tracing as acceptance condition, secrets outside the repo forever, impact in decisions, numbers never handwritten), `scripts/fitness/check_spec_kit_governance.py` in `ci.yml:guardrails`, `CLAUDE.md §0.17`, decision D-265 + issues ISS-182→ISS-185. |

| **Governance enforcement + one topic authority** | ✅ **D-266** | Two roots, both measured rather than asserted. **(1) ISS-186 — an enforcer with no target.** The repo demands `import + call chain + runtime evidence` of every capability and demanded none of it from its own enforcers: of 70 gates on disk, **seven** were run by no workflow and no test (`check_contracts_verified`, `check_core_kernel_env_profile`, `check_import_boundaries`, `check_legacy_routes_monotonic`, `check_legacy_traffic_zero_window`, `check_secret_key_consistency`, `check_stategraph_runtime_backbone`) — every one of them green when run by hand on Python 3.12. `check_spec_kit_governance` (L3) checks mention + existence, never execution, so a dead gate passed the "no orphan enforcer" check while enforcing nothing; and `run_fitness_gates.py` reads the workflow, so an unlisted gate was dead locally too. Fixed by `docs/governance/CONSTITUTION_REGISTRY.json` + `scripts/fitness/check_governance_registry.py` (execution derived by scanning workflows **and** tests, never written down; unwired debt **empty**; two proven negative tests: unwiring a gate ⇒ red, pointing a row at a deleted file ⇒ red), all seven wired into `guardrails`, L1 tightened so the three spec homes are required together (`docs/specs/` was named in D-265 §5 and did not exist), and **four phantom enforcers** corrected — including `check_curriculum_single_source`, cited in `shared/curriculum/registry.py` as D-193's enforcer and **never written**, which is precisely why three topic taxonomies could drift into ISS-159. **(2) ISS-159 — the topic hijack.** `if _c is not None and _c.canonical_id != "probability"` in **six** places over a retrieval registry knowing three maths topics and zero physics: a physics question returned `None`, the condition collapsed at its first leg, and the guard never ran. Fixed by `app/services/capabilities/topic_authority.py` (one predicate), `classify_subject`/`SUBJECT_MARKERS` in `shared/curriculum` (one home, word-boundary matching, markers normalized at import), a guard at the **entry gate** rather than in each builder, notation resolved only inside its own subject, and `check_topic_authority_single_source.py` making a seventh copy impossible (debt **empty**). Also removed `r"\b165\b"`/`r"\b14\b"` — raw strings inside `m in t`, i.e. substring matching, so D-251's word-boundary comment described behaviour the code never had. **Live proof (2026-08-16):** local PostgreSQL 16 + monolith :8000 + the real OpenRouter chain over WebSocket — `scripts/e2e/universal_answerability_live.py`, **14/14 questions answered, zero violations** across maths, physics, natural sciences, Darija and French, one terminal frame each, 28 rows persisted. Red-before-green recorded: before the fix «اشرح لي قانون أوم» returned «نسحب 3 كرات دفعة واحدة من كيس…». Contract: `tests/transcripts/iss159_physics_not_probability.yaml` (9/9 transcripts green). Production run available on demand via `.github/workflows/live-e2e.yml` (`workflow_dispatch`, deliberately outside `required-ci`). |
