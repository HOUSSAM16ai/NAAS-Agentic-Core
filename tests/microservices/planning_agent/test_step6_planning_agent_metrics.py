...
DASHBOARD = (
    ROOT / "observability" / "grafana" / "dashboards" / "90-microservices-step6-planning-agent.json"
)
...
class TestRequirements:
    ...
    def test_prometheus_client_version_pinned(self):
        ...
        assert version >= (0, 20, 0), (
            f"prometheus-client يجب أن يكون >= 0.20.0، الموجود: {match.group(2)}"
        )
...
class TestPromMetricsModule:
    ...
    def test_set_startup_info_defined(self):
        content = PROM_METRICS.read_text()
        assert "def set_startup_info" in content, "set_startup_info() غير معرَّف في prom_metrics.py"
...
class TestMainPyMetricsEndpoint:
    ...
    def test_prom_metrics_imported(self):
        ...
        assert "prom_metrics" in content, "prom_metrics لم يُستورد في main.py"

    def test_metrics_route_defined(self):
        content = MAIN_PY.read_text()
        assert '"/metrics"' in content, "/metrics route غير معرَّف في main.py"

    def test_export_prometheus_text_called(self):
        content = MAIN_PY.read_text()
        assert "export_prometheus_text" in content, "export_prometheus_text() لم يُستدعَ في main.py"

    def test_set_startup_info_called_in_lifespan(self):
        content = MAIN_PY.read_text()
        assert "set_startup_info" in content, "set_startup_info() لم يُستدعَ في lifespan"
...
class TestSupervisorSh:
    ...
    def test_planning_port_8002(self):
        content = SUPERVISOR.read_text()
        assert "8002" in content, "المنفذ 8002 غير مذكور في supervisor.sh"
...
class TestAutomationsYaml:
    ...
    def test_planning_agent_service_defined(self):
        ...
        assert "planning-agent:" in content, "planning-agent service غير معرَّف في automations.yaml"
    ...
    def test_run_step6_tests_task_defined(self):
        content = AUTOMATIONS.read_text()
        assert "run-step6-tests" in content, "run-step6-tests task غير معرَّف في automations.yaml"
...
class TestPrometheusConfig:
    ...
    def test_planning_agent_job_defined(self):
        ...
        assert "planning-agent" in content, "job planning-agent غير موجود في native/prometheus.yml"

    def test_planning_agent_port_8002(self):
        content = PROMETHEUS_CFG.read_text()
        assert "8002" in content, "المنفذ 8002 غير مذكور في native/prometheus.yml"
...
class TestGrafanaDashboard:
    ...
    def test_dashboard_uid(self):
        ...
        assert parsed.get("uid") == "cogniforge-ms-step6-planning-agent", "Dashboard UID غير صحيح"
    ...
    def test_dashboard_references_planning_metrics(self):
        content = DASHBOARD.read_text()
        assert "cogniforge_planning" in content, "Dashboard لا يحتوي مقاييس cogniforge_planning_*"
...
class TestDockerComposeStep6:
    ...
    def test_planning_agent_service_defined(self):
        ...
        assert "planning-agent" in services, "planning-agent غير معرَّف في docker-compose.step6.yml"

    def test_planning_agent_port_8002(self):
        content = COMPOSE_STEP6.read_text()
        assert "8002" in content, "المنفذ 8002 غير مذكور في docker-compose.step6.yml"
...
class TestPromMetricsFunctions:
    ...
    def test_export_prometheus_text_returns_bytes(self):
        ...
        body, _content_type = prom_metrics.export_prometheus_text()
    ...
    def test_set_startup_info_step6_label(self):
        ...
        body, _ = prom_metrics.export_prometheus_text()
        text = body.decode()
        assert 'step="6"' in text, 'label step="6" غائب في cogniforge_planning_startup_info'
